...
3. `complete`：使用本地结果。
4. `partial`：继续使用本地结果，披露逐组件缺口；不得转云端。
5. `no_data`：只有用户本次明确授权联网、实时预检通过、日期窗口和组件清单保持一致时，才允许一次云端只读回退。
6. `read_error`、Schema、完整性、数据库变化、依赖或授权错误均失败关闭，不得归类为 `no_data`。
7. 默认组件为 `sleep,hrv,body_battery,heart_rate,stress`。活动、训练负荷、账户资料、设备设置、闹钟、认证材料和原始轨迹不在默认范围内。
...
1. 可观察值：按来源和日期逐项陈述。
2. 同期关系：只写时间共现，不写因果。
3. 不可判断事项：数据缺口、日期错位、资格不足、来源不支持。
4. 一般性建议：可选、非诊断、非强制，不替用户作训练、日程或工作决定。
