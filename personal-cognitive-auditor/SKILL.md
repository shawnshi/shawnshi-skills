...
在组装最终日志前，触发认知处方：
1. 强制使用原生 `invoke_subagent` 委派子智能体 (必须指定 `TypeName: "self"` 与 `Role: "personal-cognitive-prescription"`) 执行 `personal-cognitive-prescription` 技能，传入当前的 Core Insight 和 Strategic Diagnosis。
