...
### Phase 1: Gather
1. 读取 [config.md](<C:/Users/shich/.gemini/skills/personal-cognitive-auditor/references/config.md:1>)，确认周期、时区、语言和降级规则。
2. 读取对应 prompt：
   - [DAILY.md](<C:/Users/shich/.gemini/skills/personal-cognitive-auditor/prompts/DAILY.md:1>)
   - [WEEKLY.md](<C:/Users/shich/.gemini/skills/personal-cognitive-auditor/prompts/WEEKLY.md:1>)
   - [MONTHLY.md](<C:/Users/shich/.gemini/skills/personal-cognitive-auditor/prompts/MONTHLY.md:1>)
   - [ANNUAL.md](<C:/Users/shich/.gemini/skills/personal-cognitive-auditor/prompts/ANNUAL.md:1>)
3. **[强制前置工具调用]** 执行以下自动化数据拉取：
   - **Calendar (Past & Future)**: 强制使用 `mcp_google-workspace_calendar.listEvents` 拉取过去一个周期（用于总结日程 Context Snapshot）以及未来一个周期（用于规划 Next Tactics）的日程事件。
   - **Prior Tactics (Accountability)**: 强制使用 `grep_search` 或 `read_file` 定向扫描 `~/.gemini/MEMORY/raw/privacy/Diary/` 目录，精准提取上一周/上一月的原始 `Next Tactics` 作为本次问责表的基准。
4. 汇总四类输入能力，并标出缺口。
...
### Phase 3: 认知处方集成 (Cognitive Prescription Injection)
在准备组装最终日志并向 personal-diary-writer 移交数据前，你**必须通过调用子智能体异步生成** `personal-cognitive-prescription`。
1. 使用 `invoke_subagent` 委派子智能体执行 `personal-cognitive-prescription` 技能，避免内联执行污染主线程上下文。
2. 等待子智能体返回 4 段式处方卡片（[盲区诊断]、[处方书籍]、[靶向章节]、[作用机制]）后，将其严格塞入日志模板的 `## 今日认知处方 (Cognitive Prescription)` 槽位中。

### Phase 4: Gate
1. 交付前运行 [audit_gate.py](<C:/Users/shich/.gemini/skills/personal-cognitive-auditor/scripts/audit_gate.py:1>)。
2. Gate 未通过时，优先修复：
   - 缺失问责
   - 缺失数据缺口说明
   - 缺失核心洞察/核心矛盾
...
### Phase 4: Hand-off
1. 读取 [handoff_contract.md](<C:/Users/shich/.gemini/skills/personal-cognitive-auditor/references/handoff_contract.md:1>)。
