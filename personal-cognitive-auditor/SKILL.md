...
在准备组装最终日志并向 personal-diary-writer 移交数据前，你**必须强制调用（或内联执行）** `personal-cognitive-prescription` 规则。
1. 基于当日复盘出的摩擦点，映射高维学科，锁定 Calibre 本地库中的具体硬核书籍章节。
2. 将生成的 4 段式处方卡片（[盲区诊断]、[处方书籍]、[靶向章节]、[作用机制]）严格塞入日志模板的 `## 今日认知处方 (Cognitive Prescription)` 槽位中。
