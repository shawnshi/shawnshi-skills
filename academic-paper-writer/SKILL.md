...
| 11 | `academic-scientific-visualization` (Skill) | Parse paper data and generate publication-quality figures (multi-panel, significance annotations, journal-specific formatting) via external skill handoff | Phase 4 / Phase 7 |
...
### Figures
When the paper contains quantitative results, delegate the task to the `academic-scientific-visualization` skill to generate publication-ready figures in Python (matplotlib/seaborn) or R (ggplot2). This external skill handles complex requirements like multi-panel layouts, significance annotations, colorblind-safe palettes, and specific journal formatting (Nature, Science, IEEE, APA 7.0). Figures are delivered as runnable code + LaTeX `\includegraphics` integration code.
...
academic-paper + academic-scientific-visualization -> Publication-ready scientific figures generation
```
