...
仅供 hit-weekly-brief、hit-lectures-scout 与 hit-industry-radar。自动归档权限不等于扩大路径、权限或发布范围的权限；用户要求草稿/预览/不保存时不运行 commit。不能唯一解析既有 MEMORY 工作区或用户目标时先停止，不创建平行归档目录。本规范不授权改写既有报告或 ACL。
...
3. 保存完整 stdout 为独立临时 UTF-8 JSON 文件，不手工改计划字段。检查源/目标、skill/周期/档期（radar 另含窗口模式、范围、时区）、源 SHA-256、目标和父目录快照及自定义选项符合任务；随后用同一个精确 allow-target 提交。计划不是放宽路径或权限的授权。

下面为 PowerShell 调用骨架；变量须事先解析为本次任务的绝对路径，`$planFile` 位于会话临时目录。`$skillName` 只能是 `hit-weekly-brief`、`hit-lectures-scout` 或 `hit-industry-radar`。不要把提示文本或错误输出写成计划。

```powershell
$planJson = & python -B -X utf8 "$archiveScript" validate --source "$draft" --target "$target" --allow-target "$target" --skill "$skillName"
...
& python -B -X utf8 "$archiveScript" commit --plan "$planFile" --allow-target "$target"
if ($LASTEXITCODE -ne 0) { throw "BLOCKED: preserve receipt and recovery evidence" }
```

实际 CLI：

- `validate --source FILE --target FILE --allow-target FILE --skill {hit-weekly-brief,hit-lectures-scout,hit-industry-radar}`；可选 `--allow-custom-filename`、`--allow-custom-period`，只在用户明确指定时使用。后者仅用于 weekly 自定义周期，scout 保持按其窗口元数据处理；radar 显式拒绝该参数，使用 `窗口模式`，不将其传给 radar 校验器。
...
- 自有 radar 校验器：同样五个必需参数，可选 `--window-mode {rolling7,natural_week,explicit}`（默认 rolling7）、`--report-timezone`（默认 Asia/Shanghai）、`--allow-custom-filename`；没有 `--allow-custom-period`。

共享 validate 从报告头提取参数并再次调用自有校验器；草稿临时文件名可不同，但默认目标仍须为身份指定的 DHWB/DHLS/DHWB-Radar 文件名（radar 日期恒为周期结束日）。结构通过不证明 DOI 真实、临床证据强度、原文链接可访问或日期行在同日 cutoff 之前；仍需人工来源门禁。

## 新版身份与既有报告

三个技能都要求唯一的 `报告周期：start 至 end`、`出刊日期`、带时区 `生成时点`、`报告时区`。scout 还要求唯一 `命名依据：explicit/generated`。radar 还要求唯一非空的 `窗口模式：rolling7/natural_week/explicit`、`报告范围`、`检索状态：complete/partial/blocked`，出刊日期等于周期结束日，标题为 `# 医疗行业雷达｜start 至 end`。

替换身份包括技能、周期起止、出刊日期和 scout 命名依据；radar 另绑定窗口模式、去首尾空白的报告范围和报告时区原值，不合并时区别名。既有目标还须具有对应技能的规范标题/命名身份。自定义文件名不豁免身份，不允许跨技能覆盖。

radar 只有 complete 或披露缺口的 partial 新源可归档；partial 覆盖说明、信息缺口与恢复条件仍须人工核验。blocked 诊断即使通过结构校验也不能作为归档源。检索状态不是替换身份；已有合法同身份的 blocked 诊断可被合格新源替换，但未知状态、缺失/重复元数据、非法模式/时区、scout 命名元数据或标题伪装均拒绝。
...
- 若文件当前描述符已与期望策略逐字相等，保留原生结果，不重复调用权限 setter；重复设置未保护 DACL 可能添加 AUTO_INHERITED 控制位。只有精确相等才可跳过设置；不等时仍走原生设置及严格回读比较，不忽略控制位、ACE 顺序或 protection，后续访问检查与提交后验证不省略。
