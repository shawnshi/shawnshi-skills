...
### 0.2 龙虾架构增强
1.  **感知层 (Sense)**: 集成 `history_manager.py`，执行 **SemHash (语义去重)**。若某篇论文已在过去 14 天内被扫描过，除非有重大二阶评论，否则强制拦截。
...
### Phase 1: 并发调度与 SemHash 去重 [Mode: PLANNING]
1. **Orchestrator 调度**: 利用并行函数调用同时触发 `global_scout` (Nature/JAMA)、`preprint_scout` (Arxiv)、`scholar_scout`。
2. **逻辑补位**: 若顶级正刊论文不足，必须从 `preprint_scout` 提取 5 个热点趋势补齐信息密度。
3. **SemHash 拦截**: 对比历史指纹，剔除已读文献。
...
1. **Weaver 织网**: 寻找黑板上论文与卫宁 MSL/ACE 底座或本周竞对动态的联结。
2. **Memory Interleave (MSA 增强)**: 若发现“技术落地可行性”或“历史研发背景”存在证据断层，**强制调用** `vector-lake query --interleave`。通过 `[[Action: Retrieve_Next]]` 协议，跨文档调取 L3 级冷库文档（如历史调研报告、技术规范），补齐“二跳推理”所需的物理事实。
3. **激活 Reviewer**: 调用 `activate_skill(name='logic-adversary')`。针对核心论文展开“商业化伪证”，推演其在 DRG 环境下的成本黑洞。

### Phase 4: 战略推演与杠杆锻造 (Activate) [Mode: EXECUTION]
1. **杠杆转换**: 将成果翻译为研发任务与销售话术。必须基于 Phase 3 获取的多跳证据进行事实下锚。
2. **洗稿优化**: 调用 `text-forger` 剔除学术冗余。

### Phase 5: 认知蒸馏与元数据审计 (Self-Healing & Persistence) [Mode: EXECUTION]
1. **知识入湖**: 调用 `vector-lake sync` 将报告同步至逻辑湖。
2. **元数据完整性审计 (Metadata Integrity Audit)**: **[HARD LOCK]** 严禁在最终报告中使用 `[Link]`、`[URL]` 或任何占位符。必须逐一校验每篇论文的 DOI、刊物名称与原始访问地址。若由于工具链（如 Arxiv 429 报错）导致元数据缺失，必须调用 `google_web_search` 的 `source_uris` 执行二次精准爬取，确保证据链 100% 闭环。
3. **逻辑断层审计**: 在最终交付前，执行“架构对齐度”自检。确保每一项推理均挂载了精确的 `[Ref: Evidence_Node_ID]`。
4. **物理归档**: **[MANDATORY]** 必须调用 `write_file` 将最终报告保存在 `.\MEMORY\DigitalHealthLecturesScout\` 文件夹，文件名格式为 `Weekly_DigitalHealth_YYYYMMDD.md`。
5. **技能自愈**: 将本次元数据缺失的失败先验写回 `## Gotchas`。
