...
2. **Memory Interleave (MSA 增强)**: 若发现“技术落地可行性”存在证据断层，**强制**通过 `run_shell_command` 调用 `python C:\Users\shich\.gemini\extensions\vector-lake\cli.py query ... --interleave`。跨文档调取冷库文档补齐“二跳推理”。
...
2. **知识入湖**: 通过 `run_shell_command` 调用 `python C:\Users\shich\.gemini\extensions\vector-lake\cli.py sync` 同步至逻辑湖。
