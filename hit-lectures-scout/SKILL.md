...
1. **deepxiv-sdk 脚本调用**: 通过 `run_shell_command` 执行 `python <skill_root>/assets/deepxiv_preprints_scout.py`（Windows 下解析为绝对路径）。该脚本通过 `deepxiv_sdk.Reader` API 执行：
...
**EN/CN Journals 管线 (子代理委派, 不变)**:
3. **并发调用 2 次 `generalist` 子代理**: 将 `assets/task_journals_en.md` 和 `assets/task_journals_cn.md` 的内容分别作为 Payload 指令传入。
4. **要求子代理**: 强制子代理在其独立的物理沙盒中完成"检索 -> 过滤 -> 提纯"闭环，并将结果分别写回 `tmp/playgrounds/Response_EN.md`, `Response_CN.md`。

**汇合与去重**:
5. **时序与逻辑补位**: 必须等待 deepxiv 脚本 + 所有子代理任务彻底完成后（设置 `wait_for_previous=true`），主代理才能读取三个 `Response` 文件。若顶级正刊论文不足，必须提取热点趋势补齐信息密度。
