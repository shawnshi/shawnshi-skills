...
def get_calendar_data():
    ...
    try:
        res = subprocess.run(["gws", "calendar", "events", "list", "--params", json.dumps(calendar_params)], capture_output=True, text=True, shell=False, encoding='utf-8') # SECURITY: Prevents command injection risk
        if res.returncode == 0:
