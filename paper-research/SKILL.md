...
输出结构依据 `references/template.md`。禁止参考 `{root}/MEMORY/Huggingface-Daily-Papers` 中已有论文文件的章节结构——旧文件可能使用过期模板。
...
按 Denote 规范获取时间戳，读 `references/template.md`，写入 `{root}/MEMORY/Huggingface-Daily-Papers`。
