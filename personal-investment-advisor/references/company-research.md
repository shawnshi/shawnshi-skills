...
- 带指标、数值、单位、期间、来源定位和日期的市场共识；
- 可计算的独立判断与预期差；
- 核心假设、证伪运算符、阈值、截止日期和关键变量；
- 数据来源截止日期不得晚于研究截止日期，也不得写入未来日期。

缺少任一核心项时停止深度研究，返回证据不足；不得用叙述性占位文字替代结构化数值。
...
历史 `as_of_date` 请求只有在能够取得点时快照时才可运行。当前供应商快照不得回填为历史事实；无法取得时返回 `point_in_time_snapshot_unavailable`。

数据入口：

- 通用行情与财务：`yf.py`；请求历史序列时同时传入 `--market` 与 `--asset-type`，或通过已验证组合绑定同等身份；
