...
- 产出按声明的 `decision_scope` 分三级，且标注必须与实际内容一致：`research_only`（仅结论，不含方向、价格或仓位指令）、`advisory`（含方向性判断与买卖倾向，不含可被执行参数）、`actionable`（含可被执行的方向、价格、数量或仓位参数）。高标（把 `research_only` 内容标成 `actionable`）与低标（把 `advisory`/`actionable` 内容标成 `research_only`）同为契约违反。未声明时默认 `research_only`。
- 主动研究链路仍须先通过 P0 Alpha 推广门禁才可产出候选或目标权重：`candidate_weight`、`allocation_gap` 限用于 `research_only`/`advisory` 产出，`target_weight` 仅可用于 `actionable` 产出。不得改名绕过门禁，也不得把 `experimental_weight` 直接升级为 `target_weight`。
