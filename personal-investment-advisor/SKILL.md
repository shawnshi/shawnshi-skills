...
- **逻辑硬锁 (Logic Hard Locks)**:
  - **权重锁**: 若单票权重超过 `max_weight`，Gate 将拦截除“减仓/卖出”外的一切建议。
  - **观望锁**: 若建议为“观望”，必须包含明确的“突破”触发条件，否则拦截。
  - **止损锁**: 止损位必须严格低于当前市价。
...
- Outcome Synchronizer: [sync_outcomes.py](<C:/Users/shich/.codex/skills/personal-investment-advisor/scripts/sync_outcomes.py:1>)
...
### Phase 5: Reconcile
1. 定期（建议每日或每周）运行 `decision_outcome_report.py --sync`。
2. 该操作会自动触发 `sync_outcomes.py`，抓取最新市场价格并更新 `advice_journal.jsonl` 中的 `outcome_price` 和 `outcome_return_pct`。
3. 生成的 `strategy_calibration.md` 包含绩效排行榜与校准建议。
4. 基于报告中的红黑榜（如胜率偏低或买入建议亏损），反思并调整 `stock_analyzer` 的决策参数或风控阈值。
