...
- **命令**: `$env:PYTHONIOENCODING="utf-8"; python "C:\Users\shich\.gemini\config\skills\personal-investment-advisor\scripts\quality_screener.py" --tickers AAPL MSFT ...`
...
### Phase 4: 全局量化再平衡 (Portfolio Risk Parity Rebalancing)
当用户要求“重新计算目标权重”或“整合新的配置比例”时，执行量化沙盒重分配：
...
该脚本将基于 80% A股 / 20% 美股的物理隔离约束，自动抓取一年期波动率与相关性矩阵，执行 **波动率倒数平价 (Inverse Volatility Parity)** 与 **相关性惩罚 (Correlation Penalty)**，防范 SPOF (单点故障) 与同质化仓位集中踩踏，并物理覆盖 `portfolio_positions.json` 中的 `target_weight`。
