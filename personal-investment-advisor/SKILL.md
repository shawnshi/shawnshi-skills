...
9. **生成并验证报告**：按 `references/dashboard_schema.json` 组织 `research_only` 输出，运行 `python scripts/dashboard_gate.py <dashboard.json>` 和 `python scripts/dashboard_math_gate.py <dashboard.json>`。每份 Dashboard 都必须携带已经通过门禁且 `decision_scope=research_only` 的 `research_brief`；只输出证据核验、证伪检查和观察指标。
