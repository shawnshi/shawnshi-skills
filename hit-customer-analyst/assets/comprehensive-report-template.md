...
会前速览的用户正文必须放入本报告唯一的 briefing:start 与 briefing:end 注释标记之间，随总报告正文哈希接受审核；初始化会追加速览模板。只从该区导出，不另写未审核正文。本文件其余部分保留必要审计。标准拜访包和战略客户包展开判断链、G-C-P、机会资格和执行计划。第2、8、9节属于审计区，转换为面向高层的Word/PPT时可放附录，但不得从权威Markdown删除。
...
填写说明：due_date只填有效YYYY-MM-DD；“前”、时区、日终及接收条件放入“依赖”，不得丢失原始截止含义。
