...
class MetricsScopeTests(unittest.TestCase):
    ...
    def candidate(self):
        ...
        (runtime / "candidate-base.json").write_text(
            json.dumps({"workspace": str(self.root / "formal")}), encoding="utf-8"
        )
    ...
    def test_start_scope_and_unknown_tokens(self):
        ...
        self.assertEqual(
            data["measurement_scope"], "post_skill_read_to_validation_or_safe_stop"
        )
        self.assertIn(
            "Business original-source", data["counter_semantics"]["sources_opened"]
        )
    ...
    def test_only_business_reads_increment_sources_opened(self):
        data = self.call(
            "record", "--event", "hash_bytes_read=8", "--event", "rule_read=3"
        )
        ...
        self.assertEqual(
            data["event_counts"],
            {"hash_bytes_read": 8, "rule_read": 3, "business_source_open": 2},
        )
        self.assertIsNone(data["counters"]["input_tokens"])
    ...
    def test_snapshot_scope_schema_and_legacy_compatibility(self):
        schema = json.loads(
            (SCRIPTS.parent / "schemas/run-metrics.schema.json").read_text(
                encoding="utf-8"
            )
        )
        ...
        def validate_contract(value):
            ...
            self.assertLessEqual(
                set(schema["properties"]["counters"]["required"]),
                set(value["counters"]),
            )
            if "measurement_scope" in value:
                self.assertIn(
                    value["measurement_scope"],
                    schema["properties"]["measurement_scope"]["enum"],
                )
        ...
        self.assertEqual(before, self.session.read_bytes())
        snapshot = json.loads(
            (candidate / "runtime/run-metrics.json").read_text(encoding="utf-8")
        )
        validate_contract(snapshot)
        self.assertEqual(
            snapshot["measurement_scope"], "post_skill_read_to_precommit_snapshot"
        )
        ...
        for key in (
            ...
            session.pop(key)
        session["counters"]["input_tokens"] = (
            0  # Unobserved compatibility zero is unknown.
        )
        ...
        self.assertEqual(
            migrated["measurement_scope"], "legacy_recorded_start_to_precommit_snapshot"
        )
    ...
    def test_known_tokens_require_explicit_observation(self):
        data = self.call(
            "record", "--count", "input_tokens=12", "--count", "output_tokens=0"
        )
    ...
    def test_legacy_attach_preserves_counts_without_inventing_semantics(self):
        ...
        counters = json.loads(self.session.read_text(encoding="utf-8"))["counters"]
        ...
        self.assertNotIn(
            "Business original-source", snapshot["counter_semantics"]["sources_opened"]
        )
        self.assertIn(
            "Default event zeros are unmeasured",
            snapshot["counter_semantics"]["events"],
        )
        self.assertEqual(
            snapshot["coverage_exclusions"],
            ["before_recorded_start", "commit_and_post_snapshot_work"],
        )
...
class MetricsActivationContractTests(unittest.TestCase):
    def test_read_only_loading_does_not_start_metrics(self):
        # This is a local loading/contract check, not an LLM permission sandbox.
        with tempfile.TemporaryDirectory() as temp:
            output = Path(temp)
            before = list(output.iterdir())
            for relative in ("SKILL.md", "references/candidate-workflow.md"):
                text = (SCRIPTS.parent / relative).read_text(encoding="utf-8")
                self.assertIn("获授权的实际运行", text)
                self.assertIn("已确定独立计量文件的路径及写入范围", text)
                self.assertIn("只读预览或审计不得启动计量", text)
            self.assertEqual(list(output.iterdir()), before)
            self.assertFalse((output / "session.json").exists())

    def test_test_text_io_declares_encoding(self):
        import ast

        for path in (SCRIPTS.parent / "tests").glob("*.py"):
            for node in ast.walk(ast.parse(path.read_text(encoding="utf-8"))):
                if (
                    ...
                    with self.subTest(path=path.name, line=node.lineno):
                        # Explicit other encodings remain allowed for fault fixtures.
                        self.assertTrue(
                            any(key.arg == "encoding" for key in node.keywords)
                        )
