...
class CandidateRevisionTests(unittest.TestCase):
 ...
 def test_delivery_rejects_each_missing_section(self):
  ...
  for heading in (
   ...
   with self.subTest(heading=heading), self.assertRaises(ValueError):
    v.validate_briefing_content(damaged)

 def test_delivery_rejects_empty_owner_invalid_date_missing_question(self):
  for old, new in [
   ("合成负责人（客户岗）", "待确认"),
   ("2026-09-11", "2026-02-30"),
   ("3. 下一步如何确认？", ""),
  ]:
   with self.assertRaises(ValueError):
    v.validate_briefing_content(BRIEF.replace(old, new))
 ...
 def test_metrics_entry_no_overwrite_and_actual_finish(self):
  ...
  with tempfile.TemporaryDirectory() as temp:
   ...
   self.assertEqual(
    run_python(
     "run_metrics.py", ["record", str(p), "--count", "queries_executed=2"]
    ).returncode,
    0,
   )
   self.assertEqual(run_python("run_metrics.py", ["finish", str(p)]).returncode, 0)
   data = json.loads(p.read_text(encoding="utf-8"))
   self.assertEqual(data["counters"]["queries_executed"], 2)
   self.assertIsNone(data["counters"]["input_tokens"])
   self.assertEqual(data["observed_counters"], ["queries_executed"])
...
class AuditCommitTests(unittest.TestCase):
 def test_metrics_submitted_and_mismatched_run_rejected(self):
  ...
  with tempfile.TemporaryDirectory() as temp:
   ...
   self.assertEqual(
    run_python(
     "run_metrics.py", ["attach", str(session), "--candidate", str(c)]
    ).returncode,
    0,
   )
   metric = c / "runtime/run-metrics.json"
   expected = metric.read_bytes()
   data = json.loads(expected)
   data["run_id"] = "wrong-run"
   metric.write_text(json.dumps(data), encoding="utf-8")
 ...
 def test_candidate_identity_change_does_not_touch_formal(self):
  with tempfile.TemporaryDirectory() as temp:
   ...
   ws = build_pending_strategy_workspace(root / "formal")
   c = b.prepare(ws, root / "candidates")
   path = next(c.glob("*机构研究报告.md"))
   original = path.read_text(encoding="utf-8")
   path.write_text(
    original.replace("customer_id:", 'customer_id: "other"\noriginal_customer_id:', 1),
    encoding="utf-8",
   )
   before = (ws / "runtime/manifest.json").read_bytes()
   with self.assertRaises(ValueError):
 ...
 def test_candidate_hardlink_rejected_before_formal_write(self):
  ...
  with tempfile.TemporaryDirectory() as temp:
   ...
   ws = build_pending_strategy_workspace(root / "formal")
   c = b.prepare(ws, root / "candidates")
   original = next(ws.glob("*机构研究报告.md"))
   before = original.read_bytes()
   target = c / original.name
   target.unlink()
