...
4. `invoke_subagent` (强制：拉起 cognitive-logic-adversary 进行红队攻击)
5. `write_to_file` (将沙盒草稿落盘)
6. `run_command` (执行 brief_gate.py 与 buzzword_auditor.py 联合质检)
7. `write_to_file` (终稿落盘)
8. `invoke_subagent` (调用 Vector-Lake-Ingestor 子代理异步入湖)
...
2. 调用防爆脚本与合规审计（需显式传入草稿路径）：
   ...
   $env:PYTHONIOENCODING="utf-8"; python "C:\Users\shich\.gemini\config\skills\hit-solution-architect\scripts\buzzword_auditor.py" "<appDataDir>\brain\<conversation-id>\scratch\draft_customer_brief.md"
   ```
3. 若任何一个网关拦截器报错，主代理必须根据日志修复缺失项、无效链接或行业黑话，最多重试 2 次。
...
2. **资产异步入湖**: 提取含 `[[ ]]` 双链标记的实体，通过 `invoke_subagent` 派发给 `Vector-Lake-Ingestor` 子代理执行入湖，主代理立刻释放控制权，**严禁同步等待**。
