...
## 2025-08-04 - statistics.mean performance overhead\n**Learning:** Python`s built-in `statistics.mean` is surprisingly slow due to internal type checking and exact-ratio mathematics to handle types like Decimal and Fraction. For simple lists of floats or ints, using the built-in `sum() / len()` pattern is drastically faster (~40x-60x speedup).\n**Action:** Use `sum(lst) / len(lst)` for basic average calculations instead of `statistics.mean` when strict exact-ratio arithmetic is not required.
