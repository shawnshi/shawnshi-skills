...
## 2026-07-04 - Avoiding pandas apply with axis=1 for performance
...
## 2026-07-04 - Avoiding pandas apply with lambda for simple string extraction
**Learning:** Using `.apply(lambda x: ...)` for basic string or datetime extraction (like splitting dates) in Pandas DataFrames executes row-by-row in Python and is highly inefficient.
**Action:** Use vectorized methods like `pd.to_datetime(df['col']).dt.strftime('%Y-%m-%d')` or `df['col'].astype(str).str[:10]` which are much faster.
