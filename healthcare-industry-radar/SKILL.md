...
2. **调用 `china_hit_scout`**: 侦察东软、东华、创业等国内友商的抢单动作与大模型落地。
3. **调用 `winning_health_baseline`**: 侦察卫宁健康自身的最新动作，建立己方防线基石。

### 第二阶段：交叉验证与去水 (Data Validation)
- 等待 3 个哨兵的数据回传。
- 🔴 **红队约束**: 检查每一条资讯的时间戳是否属于“本周”。如果哨兵用旧闻充数，立刻剔除。如果某一阵营本周无动作，接受“留白”，不可编造。

### 第三阶段：S-T-C 战略推演 (The S-T-C Deduction)
*指令：本阶段极其重要。你必须读取缓存中的 `references/stc_framework.md`。结合搜集到的真实情报，代入卫宁健康高管视角进行推演。*

- **Signal (信号)**: 剥离公关词汇，用一句话还原友商动作的商业实质（如：不写“开启战略合作”，写“通过0元中标拿下某三甲医院入口”）。
- **Threat (威胁)**: 剖析该动作对卫宁 WiNEX / 核心 HIS 替换周期的实质威胁。
- **Countermeasure (反制)**: 结合 `winning_health_baseline` 搜集到的己方动态，给出具体的战术应对建议（例如：用哪个子产品去阻击，或者在后续招标中如何调整标书侧重点）。
...
> [用300字概括本周最危险或最具颠覆性的行业动向。必须是基于S-T-C推演出的商业本质。]
