...
Strategy:
1. 1. 阻断裸思考：必须先通过 invoke_subagent 分离专职 research 子代抓取外部事实并回传 Payload。
2. 2. 物理隔离对抗：并发发射 5 个独立子代，分发 Payload 进行并发扫描。
3. 3. 提取分歧：把共识沉淀为公理，把矛盾提取为盲区。
4. 4. 红队校验：发射 reviewer 子代，对草稿进行交叉检验。
5. 5. 结构化入湖：结果必须调用 vector-lake-mcp 封装为 Synthesis_ 节点。
...
3. `view_file` (强制读取 references/storm_report_template.md 获取结构)
4. `invoke_subagent` (唤醒 reviewer 教授进行红队审查)
5. `write_to_file` (物理落盘最终战报)
6. `call_mcp_tool` (调用 vector-lake-mcp 执行 finalize_query_synthesis)
...
你必须调用单次 `invoke_subagent` 传入 `Subagents` 数组并发启动 5 个 `self` 子代。不可亲自串行模拟以防上下文污染。
将获取到的 Fact Payload 作为上下文写入它们的 `Prompt` 中，并赋予不同的指令要求：
...
在开始起草前，强制调用 `view_file` 读取 `C:\Users\shich\.gemini\config\skills\cognitive-storm-research\references\storm_report_template.md`。
然后起草高密度综述（暂不向用户输出）：
