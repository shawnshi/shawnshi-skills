...
description: 用于生成并保存个人日记、个人周/月/季度审计或 Mentat 日志；精确请求“更新个人日志”在完整正文通过内容门后自动保存到 canonical 季度日志，周期审计和 Mentat 走各自受保护请求门，其他个人日记仍执行确认门。
---

# 个人日志权威实现

本文件是当前安装目录的个人日志 authority。`authority.json` 使用相对定位绑定本文件；复制或安装到新目录时必须在目标目录重新生成并验证哈希绑定。
...
- 受保护用户事件的完整文本精确为 `更新个人日志` 时，允许在完整八章节个人日记生成并通过内容门后自动保存到当日 canonical 季度日志；允许单一 `[OVERRIDE]` 或 `[WARROOM]` 前缀。调用层必须提交 `personal-diary-request-v1` artifact，绑定用户事件 ID 与文本 SHA-256、日记日期、kind、专用动作 `replace-personal-diary`、canonical 目标、scope hash、payload hash 和 `canonical_autosave` 策略。包含额外修饰的句子、元讨论、草稿、预览、只读、不保存、跨日期复用或调用方自声明均不得写入。其他个人日记继续使用 hash-bound 用户确认门。
...
- 写后重新读取目标日期，日期标题数量必须等于 1；个人日记自动保存要求完整 payload 与目标日期块一致，周期审计保存还要求目标周期标题数量等于 1、原日记与其他周期审计保持不变，且请求、授权与写入范围哈希必须相等。

## Canonical 目标与写入协议

- canonical 根目录默认使用当前用户主目录下的 `MEMORY/raw/privacy/Diary`；只有安装者明确设置绝对路径环境变量 `PERSONAL_DIARY_ROOT` 时才改用该目录，不得在源码保存用户名或机器专属路径。
- canonical 个人日志：`<canonical-root>/YYYY-QN.md`；canonical Mentat 日志：`<canonical-root>/mentat_audit/YYYY-QN_Audit.md`。
- 受保护会话与跨技能门默认从当前用户的 `~/.pi/agent/sessions` 和 `~/.pi/agent/skills` 解析；非默认安装可分别设置绝对路径 `PI_SESSION_ROOT`、`PI_SKILLS_ROOT`，或更窄的 `MENTAT_EVIDENCE_GATE`、`PERSONAL_AUDIT_GATE`、`PERSONAL_AUDIT_TOPOLOGY`。所有路径仍须通过既有 canonical、根内和哈希检查。
...
- 需确认的普通日记和 Mentat 使用 `action=replace-date`；精确请求 `更新个人日志` 使用专用 `action=replace-personal-diary`。两者都在同日已有内容时整体替换该日期块，同日不存在时插入到首个日期块之前。周期审计使用对应 `replace-*-audit` 动作，只替换匹配周期的 H2 区块，并验证同日其他内容 hash 不变。payload 的第一个非空行且唯一 H2 必须为目标周期标题，其余标题只能为 H3 或更深；0–3 个空格缩进的 ATX H1/H2 与 Setext H1/H2 同样禁止。日期块不存在时只创建日期标题与目标审计区块。
...
- drafts/previews 不写入。
- 除精确 `更新个人日志` 自动保存例外外，personal diaries、custom paths、knowledge bases、Vector Lake、STQM、外部系统和任何第二处持久化必须分别取得明确授权。
- 普通个人日记必须先展示固定正文、canonical 目标、日期、动作和完整 `authorization_scope_sha256`，并要求用户回复“确认写入 <该 SHA-256>”或“确认保存 <该 SHA-256>”。调用层只保存该用户消息的 Pi session event ID；写入器从受保护会话记录核验角色、完整文本和 scope hash。正文、目标、日期、动作或 nonce 变化时，旧 scope 与 approval 同时失效。
- 本次用户只提供“今日”和“明日”事项时，不得扩写为已交付、已验收或已形成结论；日历只能证明安排。

## Canonical personal-diary auto-save exception

只有受保护用户事件精确为 `更新个人日志`、`[OVERRIDE]更新个人日志` 或 `[WARROOM]更新个人日志` 时，才生成完整个人日志并自动保存到事件发生日的 canonical 季度文件。写入器不使用 deny 正则推断授权，而是执行精确字符串相等、事件日期相等和 `personal-diary-request-v1` 全字段绑定。当前这类“修改技能，使某命令自动保存”的元请求不会被识别为日记写入授权。
...
- **采集审计**：`sync_eligible=...; sync_attempted=...; task_status=...; local_reread=...; live_fallback=...; reason=...`
...
4. 生成固定正文并执行事实、敏感信息、模板和日期标题检查；精确 `更新个人日志` 必须生成上述完整八章节并通过严格内容门。
5. 调用 `diary_ops.py scope` 并保存 scope 回执；此时状态只能是 `awaiting_confirmation`。自动保存个人日记使用专用 `replace-personal-diary`，不得借用 `replace-date` 绕过请求门。
6. 普通个人日记展示正文、范围和完整 scope hash 后等待 hash-bound 确认；三个 canonical 自动保存例外必须先取得各自来源门禁回执。调用层随后生成绑定 scope hash 的独立 approval artifact。个人日记、Mentat evidence 与周期审计 payload 各只读取一次，核验 hash 后写入私有不可变临时快照，固定门禁只读取该快照；个人日记与周期审计还要绑定受保护用户事件。随后写入阶段仍复核原始输入 hash，阻断 TOCTOU。
7. 用完全相同的内容文件、scope 和 approval 调用 `diary_ops.py replace`。需确认的普通日记与 Mentat 使用 `replace-date`；自动保存个人日记使用 `replace-personal-diary`；周、月、季度审计分别使用 `replace-weekly-audit`、`replace-monthly-audit`、`replace-quarterly-audit`。
8. 复读目标日期，验证日期标题恰好 1 个、个人日记日期块或目标周期区块等于 payload、目标周期标题恰好 1 个（如适用）、请求/授权/写入范围摘要一致，且非目标历史未变化。
