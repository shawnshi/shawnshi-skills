...
根入口选择私人数据读取分支后、任何私人读取前完整读取本文件，并先通过根入口启动门；正式个人日记的保存分支必须先执行本合同的默认读取；用户明确排除来源或仅用当次文本时，才可仅查阅能量模板，不因此获取额外采集授权。纯当次用户文本草稿不加载本合同、不采集数据。限定默认授权不覆盖用户明确排除的数据源；日记历史只读请求所需目标日期块，不扩展扫描。读取授权不是日记保存授权；只读/不保存请求不生成写入回执。
