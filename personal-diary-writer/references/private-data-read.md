...
根入口选择私人数据读取分支后、任何私人读取前完整读取本文件，并先完成根入口的用户授权和范围检查；正式个人日记的保存分支必须先执行本合同的默认读取；用户明确排除来源或仅用当次文本时，才可仅查阅能量模板，不因此获取额外采集授权。纯当次用户文本草稿不加载本合同、不采集数据。限定默认授权不覆盖用户明确排除的数据源；日记历史只读请求所需目标日期块，不扩展扫描。读取授权不是日记保存授权；只读/不保存请求不生成写入回执。
...
- 正式个人日记默认必须进入私人数据读取分支，即默认授权只读获取日记日期及次日日历的最小事件字段，以及最近 3 天 Garmin 健康摘要；窗口按 Asia/Shanghai 取日记日期 D-2 至 D，含当日共 3 个自然日。仅提供今日/明日事项不是跳过理由；仅用当次文本或排除来源须由用户明确指定，读取门失败须保留真实错误，不得伪装成无数据；不再为这两类限定读取重复询问授权。仅在未命中上述同步退出条件、窗口含当前自然日且必需组件末端陈旧时，还授权直接运行一次两阶段同步（`sync_health_data.py sync --dry-run` 后 `--allow-network --allow-sync`），不再启动 `Codex-Garmin-Health-Sync` 计划任务。权限不足或授权失败不得同步，也不授权注册或修复任务。
- Garmin 必须本地优先，并先通过 canonical `personal-health-analysis` 的依赖预检与数据授权检查。未命中同步退出条件且末端陈旧时直接运行两阶段同步一次并重读本地；同步失败保留同步前证据，不重试、不改走实时接口。仅当本次未尝试新鲜度同步且本地明确返回 `no_data` 时，才可使用已有有效凭证对同一 3 天窗口执行一次云端只读查询；`partial` 继续使用本地数据并披露缺口，依赖、Schema、完整性或其他读取错误不得触发云端回退。
...
1. 正式个人日记默认必须只读获取日记日期及次日日历的最小字段，以及最近 3 天 Garmin 健康摘要。某一来源失败不阻止另一来源在其独立门通过后继续读取；失败关闭仅停止受影响来源，披露缺口后可按保存门保存，不声称完整采集。日历读取顺序固定如下，不得跳步或静默切换数据源：
...
只有实际读取健康摘要时才填充观测值；正式个人日记已有默认健康读取需求，不得以未单独要求健康或模板允许缺口为由跳过。用户明确排除来源或读取门失败时不强行采集；完整八章日记仍保留能量章节并注明未读取及判断边界。生成后运行适用的 `audit_gate.py --enforce-template-fields`。
