...
1. **初始化调度**: 主代理必须调用原生 `invoke_subagent` 工具，并发拉起 4 个 `TypeName: research` 类型的子代理。分别下发本技能 `assets/` 目录下的四份指令包作为 Prompt：
   ...
   并在拉起子代理的 Prompt 中明确指示：“完成任务后，使用原生 `write_to_file` 工具将底层抓取数据写入 `C:\Users\shich\.gemini\tmp\raw_brief_data_[管线].json`”。绝对禁止主代理在此阶段直接在主线程执行广域搜索。
...
1. 根据 `resources/template.md` 模板渲染极高压迫感的简报草稿，强制使用原生 `write_to_file` 工具写入临时文件 `C:\Users\shich\.gemini\tmp\draft_hit_brief.md`。
2. **防爆代码审查**: 强制调用跨平台红队脚本，挂载 UTF-8 数据流安全锁（必须执行绝对物理寻址）：
   `$env:PYTHONIOENCODING="utf-8"; python "C:\Users\shich\.gemini\config\skills\scripts\hit_audit_gate.py" "C:\Users\shich\.gemini\tmp\draft_hit_brief.md" --mode brief`
