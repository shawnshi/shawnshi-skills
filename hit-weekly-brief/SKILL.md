...
1. **Packet Creation**: Before starting a heavy ingestion task, write the specific parameters, source URLs, or target documents to a sandbox file such as `tmp/playgrounds/Digestion_Packet_[TIMESTAMP].md`.
...
0. **Initialize Blackboard**: 创建 `tmp/intelligence_blackboard.json` 共享状态。
 ...
 - `Task_strategy.md`: 目标锚定 McKinsey, BCG, Bain, Deloitte, PwC 最新研报。必须使用搜索/浏览能力交叉验证发布日期。提取战略信号。
 - `Task_policy.md`: 目标锚定 WHO, RAND, World Bank, OECD, ONC 等发布的公共卫生、数据隐私及医疗互联互通政策报告。必须使用可用的浏览或抓取手段验证日期。
...
- `tmp/playgrounds/Task_strategy.md`
- `tmp/playgrounds/Task_policy.md`
- `tmp/playgrounds/Task_tech.md`
- `tmp/playgrounds/Task_serendipity.md`
