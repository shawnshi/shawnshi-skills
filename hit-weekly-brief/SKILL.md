...
3. 职责分工与受众：周报面向中国医疗机构管理层与医疗 IT 经营决策者进行周度综合研判，以自然周（周一至周日出刊）或明确历史闭区间为界；与行业雷达（按滚动/截止时点跟踪厂商/政策/采购阶段事件）和情报 Hub（基线优先每日资讯与历史去重）明确区分。用户指定地区/受众优先；全球信息仅作可迁移参考。按合规期限、采购/运营/架构影响排序，不虚构量化收益。默认当前自然周（周一至周日），出刊日取周日；自定义周期须用户明确指定，出刊日等于结束日。历史补刊/上一完整周按用户口径。延续归档时只读最近三期文件名和标题，沿用不冲突的稳定惯例，不据此自动启用自定义；无法确认说明口径。
