...
- `direct` 及不生成文件、不使用 Blackboard 的纯对话 `brief`，按问题范围执行相称的事实与边界检查，不运行文件型门禁，也不为满足门禁创建文件。文件交付使用 `scripts/strategy_gate.py` 统一检查；无 Blackboard 的 `brief` 文件按下述 `--textual-only` 路径。`working_draft` 或 `review_ready` 可以带着明确披露的警告供复核；`decision_ready` 或其他正式决策级交付必须使用 `--strict`，确定性错误或 `blocking_warnings` 未处理时不得声称门禁通过；`advisories` 中的非阻断编辑建议不因 strict 单独失败。
- 报告中的显式证据/假设/输出 ID 必须存在；正式财务指标使用 `[[output:ID metric=roi value=50 unit=%]]` 绑定当前 Blackboard 输出。所有权威成熟度声明必须一致，示例不构成批准。门禁的 `unchecked` 始终保留散文数字、来源支持和批准真实性等人工复核范围。
...
- 转交架构或写作时保留原 ID、证据、假设、数字口径、未决项和批准边界，按 [references/workflows.md](references/workflows.md) 的最小交接字段处理；战略成熟度不自动升级为实施或发布批准。
