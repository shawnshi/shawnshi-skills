...
主代理必须使用原生的 `invoke_subagent` 工具并行拉起 Retrieval Specialist。一队穿透卫健委/医保政策，另一队穿透竞品动作。要求提取具体的定量指标（如千万级预算）和非共识观点，并在拉起子代理的 Prompt 中明确指示：“完成任务后，强制使用原生 `write_to_file` 工具将底层侦察数据写入 `C:\Users\shich\.gemini\tmp\recon_data_[管线].json`”。
