...
主代理必须使用原生的 `invoke_subagent` 工具并行拉起 Retrieval Specialist (必须指定 `TypeName: "research"`)。一队穿透卫健委/医保政策，另一队穿透竞品动作。要求提取具体的定量指标（如千万级预算）和非共识观点，**并在 Prompt 中下达硬性纪律**：“严禁直接读写物理磁盘，必须使用 `send_message` 将底层侦察摘要以 JSON Payload 回传”。主代理维持 Reactive Wakeup (响应式唤醒) 状态纯净等待。
...
3. **战略结果门拦截**：
   ```powershell
   $env:PYTHONIOENCODING="utf-8"; python "C:\Users\shich\.gemini\config\skills\hit-digital-strategy-partner\scripts\strategy_gate.py" --path "C:\Users\shich\.gemini\MEMORY\research\<project_name>\final_report.md" --mode "<mode>" --blackboard "<appDataDir>\brain\<conversation-id>\scratch\strategy_blackboard.json" --strict
   ```
...
- **交付链接契约**: 终稿合并与检验通过后，主代理必须通过聊天框向用户输出可点击的 Markdown 绝对路径链接（例如：`[医疗战略研究报告](file:///C:/Users/shich/.gemini/MEMORY/research/<project_name>/final_report.md)`），一键直达战略资产。
- **Telemetry 遥测**：使用 `write_to_file` 将包含 `skill_name`, `mode`, `topic`, `result_gate` 的 JSON 记录落盘至会话沙盒：`<appDataDir>\brain\<conversation-id>\scratch\telemetry.json`，彻底规避全局死锁。
