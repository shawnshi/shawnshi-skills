...
保存：生成的报告保存在"{root}\.gemini\MEMORY\wiki\news"
...
## 历史失效先验 (Gotchas)
- ALWAYS use `is_redundant` check before refining.
- **DO NOT** allow 'antirez.com/news/160' or similar stale benchmark URLs into Top 10. They are permanent baseline references, not news.
- **DO NOT** summarize corporate PR.
- ENSURE `urgent_signals` are restricted.
