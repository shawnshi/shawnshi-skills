...
父任务收到工具耗尽或完成事件时，在写入 `declare_lost` / `degraded_timeout` 或其他 terminal 状态之前只检查授权 draft 一次。所有父任务 fallback（完整动态或已装配 draft）均必须先在同一 frozen run 的工作目录执行绑定帮助脚本 `finalize --request <request> --gap-id <gap_id> --parent`；此命令仅确定性装配，不是代写研究，不新增子任务或检索。来源时间必须符合原预算，且仍在原 `completed_at` 后的 existing finalization grace 内；不得修改时间戳来挽救迟到尝试。已装配 draft 也必须先通过 `finalize --parent` 的 terminal / grace guard；该分支只跳过重装配，不跳过 guard，也不改字节。仅在 guarded helper 成功后才能交现有 `finalize-supplement` 正式校验登记；guard 拒绝时禁止直接登记。缺失、部分、畸形、超时或已有 terminal 的证据保留原字节并按原 reconciler 收口；helper 成功只表示 draft_ready，不表示 completed 或正式覆盖。父任务必须串行执行此收口与 terminal 状态登记；同一 gap 的 seal 与 `finalize --parent` 之间不得插入任何其他 gap 的 broker 操作，seal 时若授权 draft 已存在必须立即收口（`scripts/supplement_seal.py --request <request> --gap-id <gap_id>` 在同一条命令内完成 seal 与受 guard 收口，并在无 draft 时打印 grace 截止时间）；**父任务 fallback 先写 draft 再 seal 会被拒绝（seal guard：draft 或 result 已存在即拒绝），正确顺序是先 seal，再写 draft，再 finalize**；同一 gap 的 600 秒 source 时钟从 `broker-checkpoint` 的第一个 ledger 事件起算（不是首次 reserve）；因此不得为多条 gap 先批量 checkpoint 再逐条跑完，否则尚未开跑的车道会在 fetches 之前到期并只能走 failed reconciliation。逐条 gap「checkpoint → 跑完 → seal → 立即 finalize」是唯一不会自伤时钟的顺序。旧 run / snapshot 不重开，12 tools / 查询 / URL / 1M Token / $3 上限均不增加。
...
native readable 发布日期只认行首显式 Published / Published on / Publication date / 发布日期 / 发布时间 / 发表日期，或首个非空行的独立 dateline，或规范电讯稿声明：`PRESS|NEWS|MEDIA RELEASE <date>` 行（`wire-label/1`）与页首元数据前缀内以 `地名 — <date> —` 定界的电讯头（`wire-dateline/1`）。支持 ISO、YYYY年M月D日、英文完整/三字母月份、一或两位日、严格四位年及有效日历。电讯通道另接受四字母 `Sept` 与带序数词的「日 月 年」顺序，仅在既有标签、首行日期和 NHSA 规则均未给出日期、且既有通道未出现日期冲突时作为回退使用，不改写既有通道的月份口径；电讯行按已知元数据字段计入有界页首区域，仍不扫描首个正文句。电讯形态由同一严格判定函数供页首准入、标题排除和日期提取共用：地名区大小写敏感且任一以逗号、空白、连字符或下划线分隔的地名片段（去除允许的句点后）不得为 Updated/Modified/Last 等发布状态词，必须带破折号、括号或通配署名等电讯终止符，日期后必须是非字母数字边界（拒绝五位年份等串接），同一行日期之后（含 dateline 尾部正文）再出现任何受支持形式的带日期发布/更新时间声明（Published、Publication date、PRESS/NEWS/MEDIA RELEASE、Updated/Modified/Last updated、发布/更新时间）也一律拒绝；此类标签内部的空白仅限空格与制表符，标签与日期之间仅容许单个句点或中英文冒号，标签大小写不敏感，其他拼写变体不在合同形状内并保守不接受，残留强调符、无终止符的普通句子同样拒绝。列表与成对 Markdown 强调只解码语法并映射回原始 Unicode offsets，date raw/start/end 与完整 readable SHA 不归一化；多个声明须一致，缺年、无效或冲突封闭失败。updated/modified、正文事件、页脚、版权、当前时钟及 URL 本身不能授予发布日期。仅精确 NHSA 主机 /art/YYYY/M/D/... 的标题后、正文前 `日期：... 访问次数：...` 可由 URL 日期佐证；必须另有紧邻正文段落才授予 article，导航/视频链接不算。强发布日期后的发布机构字段不属于 date raw。标题优先 literal heading/名称字段，跳过导航及元数据（含电讯稿行与 `opens in new window` 等导航残留）；有 leading dateline 且无可用标题时，仅首段介绍中唯一非通用本地 PDF 链接的 literal label 可作 fallback，并标记 `title_source=introductory-document-link`（不声称 h1 或认证标题）。标题基准（2026-09-14 授权）：正文标题为空或不落在 8..240 时，回退使用绑定 lane 已登记 feed 标题，记 `title_source=lane-declared/1`；正文标题合法时永不回退。文章长度/段落、窗口、来源质量门不放宽；arXiv identity/version/UTC submission-history 分支不变。正文日期缺失但 `article_core`（除正文发布日期外的全部文章判据）成立时，登记可引用绑定 lane 已登记的 feed 日期（`published_at_proof.parser_rule=pool-declared/1`，2026-09-14 授权）；正文自带日期存在时禁止使用该基准，声称来源仍不得为 unknown/retrieved_at。无清晰行界的拼接发布者文本仍可能不支持。
...
发布日期格式与有界页首区域遵循上方 Article broker 的 native readable 规则（含完整/缩写英文月份、中文日期及 Markdown 包装），不扫描全文标签。日期证明包含 exact raw quote、Unicode start/end、parser_rule published-label/1、leading-dateline/1、nhsa-header-date/1、wire-label/1 或 wire-dateline/1、normalized day 和完整文本 digest。缺年份、update/event/body-mentioned 日期、区域内畸形或相互冲突的 publication 声明不作发布日期。文章要求非首页/portal/search URL、至少 400 UTF-8 字节、两个各 >=120 字符的段落、8..240 字符标题、有效 publication 且无 challenge/login 标记；仅为保守结构证据，不认证事实或 primary source。正文自带日期与声明冲突、超窗仍不能选入候选；正文无日期而其余判据成立时，仅可按 `pool-declared/1` 引用绑定 lane 已登记 feed 日期登记（2026-09-14 授权）。
...
新 v3 来源窗口为 source600秒，finalization grace900秒，launch timeout 为 900000ms；query2 / fetch4 及其他预算不变，旧冻结运行不改写。CLI 正文摘录最多 4000 JSON UTF-8 字节、metadata 最多 1000 字节，独立摘要 hash/truncation 标记；原 receipt/proof_path 完整保留。sealed exact access/date/proof → parent finalization → registration/date ownership → semantic lineage → forge/gate；不得把原生错误伪装为 HTTP permanent 或 empty search。
...
### Parent broker receipt contract (native v3, parent-only)

The parent never has to reverse-engineer these from validator errors. Two receipt schemas exist and they are not interchangeable; both must carry `parent_attestation="actual_public_tool_receipt"`.

- Fetch receipt (`broker-record-fetch`), exactly these keys plus optional `responseId` / `text_coverage`: `request_sha256`, `gap_id`, `reservation_id`, `invocation_id` (= `<request_sha256>:<gap_id>:<reservation_id>`), `tool="fetch_content"`, `arguments` (byte-equal to the reservation's), `started_at`, `completed_at`, `outcome` (`success` | `error` | `partial`), `error`, `text`, `truncated`, `parent_attestation`.
- Query receipt (`broker-record-query`), exactly these keys: `request_sha256`, `gap_id`, `reservation_id`, `tool="web_search"`, `query` (byte-equal to the reservation's), `responseId`, `outcome` (`matched` | `empty` | `error`), `error`, `results` (list of exactly `{url,title}`), `proof_subset` (non-empty), `parent_attestation`.

Binding rules that fail closed:

1. Chronology: `reservation.at <= started_at <= completed_at <= now`. Never guess these: read `reservation.at` from the ledger and take `completed_at` from the actual clock. Future or backdated values are rejected as `native receipt chronology invalid`.
2. Path separation: the receipt is an input file. `broker_<gap>_body_<n>.json` is the proof the CLI writes; passing the proof path as `--receipt` fails with `proof path already exists; never replace`.
3. Queries: `executed_queries` on the sealed ledger is a plain list of query strings and the draft must echo it verbatim, not as objects.
4. Candidate proofs: each registered candidate needs `broker_body_proof_sha256` (the proof entry's `proof_sha256`) and `published_at_proof`, byte-equal to one entry of that proof's `metadata.dates`. Only when `metadata.dates` is empty and `metadata.article_core` is true may an undated body borrow the bound lane's registered feed date, with `published_at_proof.parser_rule="pool-declared/1"` plus the lane's own `published_at_source`. A search-discovered URL has no lane declaration and cannot use that fallback; a body that already carries a date can never be overridden.
5. `finalize --parent` requires a draft at the packet's `draft_path`, and the parent fallback order is seal → write draft → finalize, never draft before seal.
6. Bound candidates are not reservable. `broker-reserve-fetch` accepts only the exact required bound URLs or URLs already discovered by a recorded search receipt; a non-required lane candidate is rejected as `URL not bound or dis[covered]`.

Recovery preconditions: `reconcile-supplement` accepts only the packet-declared `supplement_<gap>_progress_state.json` path, and no v3 CLI writes that file. A gap whose worker settles without a draft therefore has no reconciler path: the parent must author the draft itself and run `finalize --parent` inside the unchanged 300s grace, or the gap stays open and `finalize-supplement` refuses the whole request with a candidate-date-evidence result-gaps mismatch. A worker that settles BLOCKED before requesting the broker sequence is not an infrastructure failure and does not stop fanout by itself; it leaves the gap for parent execution.

## runtime

新运行不设 Token/费用总额上限（owner-authorized 2026-09-16，原 1,000,000 Token / 3 美元上限已移除）；每条补检预留 150,000，语义评审 200,000，红队 100,000，下游 headroom 合计 300,000。预留、计量与终态结算全部保留用于可观测性，但触达任一旧上限不再阻断启动。原有访问、工具、超时与结算门保留。旧快照及遥测不得按新上限改写。
...
语义代理只负责在 helper 返回的 `eligible_candidates` 中执行门禁、排序、事件身份、L1-L4、中文判断和行动建议，并只写 `semantic-dynamic/1.0` 草稿。`corroboration_policy.single_secondary_allowed=true` 时，`corroboration_status=single_secondary` 的候选就是可入选的合格候选，代理不得以“需要两个独立 ref”之类的额外暴露规则排除它们。动态草稿不得生成静态 run/hash、coverage、candidate funnel、mix、pipeline、access log、lineage或回执字段；不得重新联网访问已登记 URL。父任务在代理失联后按 fallback 补写草稿时，如果改动或追加了 `selected_items`，必须同时重写 `punchline`/`insights`/`digest`/`market` 使叙述覆盖最终选定集合，并逐条核对每个 item 的 `title_zh`/`summary_zh`/`fact` 与其自身 `url` 的已核验正文一致；不得只追加条目而沿用旧叙述，也不得把其他条目的正文归到本条目。写完后立即运行 `execution_packet.agent_helper.finalize_command`（同 `validation_command`）：
...
4. 区分事实、来源主张、分析推断、行动和未知项；一手来源可单独进入语义候选，二手来源在同一结构化事件身份下获得至少两个独立来源及访问回执时可作为 `multi_independent` 候选组；若本轮 focus config 的 `corroboration_policy.single_secondary_allowed` 为 true（2026-09-14 授权），单一独立二手来源（已验证访问、同一事件身份）可作为 `single_secondary` 候选入选，同一事件的其他同源条目仍按 `secondary_without_independent_corroboration` 拒绝；
