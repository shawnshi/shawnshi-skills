...
Strategy:
1. 1. 获取意图：确定播报内容、情绪场景和分发模式（标准/导演/多角色）。
2. 2. 环境锁定：必须使用原生
3. un_command 调用并强制挂载中文字符集安全锁与物理绝对路径。
4. 3. 双轨回退：神经网络调用失败时允许自动切回物理 SAPI5 进行降级播报。
AVOID: 禁止直接给出一段 Python 代码让用户自己去跑；禁止擅自改动多角色剧本或剥离原有的 Audio Tags。
</strategy-gene>

# Tool TTS (高保真导演级播报系统 V9.0 Native)

## Tool Trajectory
**[IN_ORDER]** 执行需遵循以下轨迹流：
1. 
2. un_command (调用 tts_engine.py 进行语音合成与物理发声)

## 1. 核心流程与架构 (The Protocol)
### Phase 1: Context & Mode Selection
- 判断播报等级，根据任务性质从 <Contracts> 中选择音色与模式（Standard, Director, Multi-Speaker）。

### Phase 2: Engine Execution (引擎激活与上锁)
- **[HARD LOCK]**：必须使用 
un_command 执行任务。为防止 Windows 终端因中文字符集而闪退截断，必须强行加挂 $env:PYTHONIOENCODING="utf-8" 与完整的绝对物理路径。
...
- **沙盒迷失 (Pathing Deadlock)**：绝对禁止大模型在终端中使用 python scripts/tts_engine.py 等相对路径。必须使用绝对物理地址，否则终端必定向系统抛出寻址异常。
