...
2.  **Context Assembly (上下文收集)**:
    ...
    *   **日程校准**: 若为 Daily Log，【强制物理调用】：必须使用 `gws calendar` 工具拉取当日及明日日程。严禁脑补。
3.  **Density Gateway (触发网关与大纲阻断)**:
    *   **若是 Daily Log**: 评估当日交互对话。如果没有实质性的会话内容，跳过深度复盘提取。
    *   **若是 Audit Workflow**: 读取相应期段的日志记录。如果无实质内容，则拦截执行。如果执行长篇复盘，【大纲拦截】：【必须强制】调用 `ask_user` 展示初步大纲并请求审批。未经核准，严禁进入起草阶段。
...
2.  **Strategic Sync (记忆蒸馏)**: 
    *   提取高深度的洞察（`cognitive_depth_score` >= 4），并调用脚本更新 `memory.md`。
...
*   ❌ **禁止脑补虚假数据**：严禁在未成功调用 Garmin 或 Calendar 插件时虚构生理指标或日程。
