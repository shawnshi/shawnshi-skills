...
## Tool Trajectory
**[IN_ORDER]** 本技能需依序匹配以下核心调用流：
1. un_command (执行 converter.py 进行降维)
2. invoke_subagent (委托 research 代理读取模板与提取)
3. un_command (执行 paper_audit_gate.py 物理审计)
4. write_to_file (最终结果落盘)
注：偏离此轨迹则视为执行越权或幻觉。
