...
**本技能采用严格的 SSL (Scheduling-Structural-Logical) 架构，必须按以下物理阶段（Phases）串行执行，严禁跳步。**

### [Phase 1: Task Delegation (调度隔离)]
- 获取目标论文：读取本地 PDF 或抓取网页。
- **强制约束**：提取标题、问题、批判对象（宿敌）等信息时，**必须**将指令封装为 Task Packet 并交由子智能体（如 `generalist`）处理，结果写入 `tmp/playgrounds/`。严禁在主上下文中硬读超过 20 页的 PDF全文。

### [Phase 2: Traceback Construction (溯源构建)]
- 根据 Phase 1 提取的 Baseline，调用搜索/阅读工具，**向上递归追溯 3-5 层**。
- **强制产出**：在脑海中或临时草稿中构建出 `[Paper A] --(问题X)--> [Paper B] --(问题Y)--> [目标论文]` 的演化链。每层只抓三个核心：标题/年份、核心解法、对前人的批判点。

### [Phase 3: Markdown Assembly (拼装草稿)]
- 按照 `resources/template.md` 的规范，拼装最终的费曼式演化叙事与博导审稿结论。
- **强制约束**：禁止直接输出给用户或使用 `write_file` 落盘到最终目录。你**必须**将这份 Markdown 草稿写入临时路径，例如 `tmp/draft_paper.md`。

### [Phase 4: The Hard Gate (物理层强制审计)]
- 这是保障逻辑严密性的硬网关。在生成临时草稿后，你**必须**调用 shell 命令执行：
  `python {SKILL_DIR}/scripts/paper_audit_gate.py tmp/draft_paper.md`
- **处理失败**：如果审计脚本报错退出（例如“缺乏必要节点”、“演化链断层”或“触发学术禁词”），你**必须**阅读报错日志，退回 Phase 3 修正 `tmp/draft_paper.md` 的内容，并重新执行本阶段的过检命令。
- **重试上限**：最多重试 2 次。若 2 次后仍未通过，必须抛出 `[Escalation_Required]`。

### [Phase 5: Commit (物理落盘)]
- **只有在 Phase 4 的脚本明确返回 `Audit Passed` 状态码 (Exit Code 0) 后**，才允许通过 `write_file` 或 shell 命令将 `tmp/draft_paper.md` 的内容真正写入最终的物理归档路径（`MEMORY/raw/Huggingface-Daily-Papers/`），并清理临时草稿。
...
- `IF [Action == "Read PDF"] THEN [Force limit read_file using end_line parameter or use grep_search to locate chapters first. Strictly prohibit unconditionally ingesting all raw text of PDF.]`
