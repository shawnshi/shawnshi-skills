...
def validate_paper_draft(content: str) -> list[str]:
    ...
    # 3. 防八股文与翻译腔扫描 (Aesthetic Defense)
    banned_phrases = [
        "值得注意的是",
        "近年来随着",
        "本文提出了一种",
    ...
    for phrase in banned_phrases:
        if phrase in content:
            errors.append(f"Aesthetic Violation: Contains banned academic or translated phrase '{phrase}'.")

    # 3.5 检查公式裸奔 (LaTeX Formula Check)
    if re.search(r'\$\$.+?\$\$', content, re.DOTALL):
        errors.append("Formula Violation: Found raw block LaTeX formulas ($$ ... $$) in the main text. They must be translated into natural language or moved to an appendix.")
