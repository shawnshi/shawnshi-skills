...
**1.5 Pre-flight Check (Auth & Env) ⚠️**

**Purpose**: Verify local environment and private authentication.

1. **Check Config**: Look for `EXTEND.md`. If missing, run First-Time Setup.
2. **Check Auth**: Scan skill directory for `.env`.
   - If `GOOGLE_API_KEY` exists → **Private Auth Mode** (Inject keys to executor).
   - If missing → **Global Tool Mode**.
3. **Check Platform**: Detect Win/Bash to ensure `New-Item` vs `mkdir` compatibility.
...
**Automation Strategy**:
1. **Batch Execution (Private)**: If in *Private Auth Mode*, use the resilient executor for each illustration in the outline:
   ```bash
   python scripts/executor.py "[prompt]" "[output_path]"
   ```
   *Note: This script automatically finds the best available model (Imagen 4.0, etc.) from your .env key.*
2. **Sequential Processing**: Generate images one by one, reporting progress (e.g., "Generated 2/5").
3. **Retry Logic**: The script handles model fallbacks; only fail Step 5 if all models in the key's quota return errors.
