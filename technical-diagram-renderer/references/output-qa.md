...
- 讨论草图的 ER 基数、PK/FK、状态转换和用例关系逐项对照用户输入或授权资料；未确认项明确标为假设或待确认，不自行补成事实。正式设计、竣工、验收、合规图及用户指定批准的情况，仍须业务或技术负责人确认。
...
讨论草图须通过与本次交付相关的自动、视觉和格式复核，拓扑与用户输入或授权资料一致；明确标注并随交付列出的假设、待确认关系不阻止“可交付讨论草图”，但不得标成拓扑已确认或正式设计。用户明确要求先批准的事项仍须等待。正式设计、竣工、验收或合规图须完成全部适用复核及相应责任人审核。
