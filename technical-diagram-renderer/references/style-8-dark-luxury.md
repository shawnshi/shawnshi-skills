...
> **状态：仅保留的实验性设计参考。** 当前 `generate-from-template.py` 明确拒绝 Style 8 和 `dark-luxury`。不要把它列为可生成选项，也不要手工拼接 SVG 后声称通过了本技能的自动验证。
...
**未来启用条件**：只有在该主题进入输入白名单、安全净化、回归样例和视觉 QA 后，才可用于正式生成。启用前，本节中的代码片段不能作为交付路径。
