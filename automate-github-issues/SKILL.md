...
附带本地脚本与手动合并模板使用同一入口：显式绑定获批仓库、base、任务、PR 和完整 head SHA，核对受信 `.fleet` 同一记录中的任务/session/repo/PR 编号绑定与完整 session token；PR 归属绑定须独立核验并明确提供，不能从候选 PR 正文或分支自述建立，缺失或不符时在 GitHub 调用前停止；仅非空必需 CI 全部成功、无冲突且 head 未变时按 SHA 合并。模板默认 dry-run，不扫描所有机器人 PR，不自动更新分支、关闭或重分派；缺受信映射、无必需 CI 或冲突时停止。受信记录只能来自已获准基线或明确提供的输入，不从候选 PR checkout 建立信任。记录提供、关闭及重分派均须各自授权，新 head 需重新批准；参数和前置条件见 README。
