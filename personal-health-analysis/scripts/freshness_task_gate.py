...
def _powershell_probe(script: Path, task_name: str, mode: str) -> dict:
    ...
    return _parse_json(completed.stdout.strip())
...
def validate_task(snapshot: dict, expected: dict) -> str | None:
    if not snapshot.get("ok") or not snapshot.get("exists"):
        return "task_missing"
    checks = (
...
def validate_terminal_state(state: dict | None, end: str, authority: dict, expected_run_id: str | None) -> str | None:
    if not state:
...
def main() -> int:
    ...
    try:
        code, audit = run_gate(args, probe=probe, state_reader=lambda: _read_state(Path(args.state_output)))
