...
def analyze_flu_risk(summary_data):
    ...
    # ⚡ Bolt Optimization: Use walrus operator to avoid redundant dictionary .get() lookups in list comprehensions

    # Calculate simple baseline (avg of previous days)
    prev_hrv = [val for d in hrv_data if (val := d.get("last_night_avg")) is not None and val and d != latest_hrv_entry]
    prev_rhr = [val for d in hr_data if (val := d.get("resting_hr")) is not None and val and d != latest_hr_entry]
    prev_resp = [val for d in sleep_data if (val := d.get("avg_respiration")) is not None and val and d != latest_sleep]
    ...
    prev_waking_resp = [val for d in daily_summary_data if (val := d.get("rr_waking_avg")) is not None and val and d != latest_daily]
...
def calculate_sleep_consistency(sleep_data):
    ...
    durations = [val / 3600 for s in sleep_data if (val := s.get("sleep_time_seconds")) is not None and val]
    if len(durations) < 2:
        return 0, "数据不足"
...
def analyze_executive_readiness(summary_data):
    ...
    prev_rhrs = [val for h in hr_data if (val := h.get("resting_hr")) is not None and val and val != latest_rhr]
...
def generate_chinese_insight(summary_data):
    ...
    if len(hr_data) >= 4 and len(stress_data) >= 4:
        ...
        first_half_rhr = statistics.median([val for h in hr_data[:mid_point] if (val := h.get("resting_hr")) is not None and val])
        second_half_rhr = statistics.median([val for h in hr_data[mid_point:] if (val := h.get("resting_hr")) is not None and val])
        
        first_half_stress = statistics.median([val for s in stress_data[:mid_point] if (val := s.get("avg_stress")) is not None and val])
        second_half_stress = statistics.median([val for s in stress_data[mid_point:] if (val := s.get("avg_stress")) is not None and val])
