...
def analyze_baseline_change(summary_data: dict[str, Any]) -> dict[str, Any]:
    ...
    # Performance: Replaced slow statistics.mean with built-in sum/len for significant speedup
    baseline_hrv = sum(prev_hrv) / len(prev_hrv)
    # Performance: Replaced slow statistics.mean with built-in sum/len for significant speedup
    baseline_rhr = sum(prev_rhr) / len(prev_rhr)
