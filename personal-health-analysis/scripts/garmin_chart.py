...
def build_overlay_data(summary_data):
    ...
    try:
        ...
        # Performance: Replace iterrows() with zip() for vectorized iteration, avoiding Series instantiation overhead.
        pmc_map_ctl = {d: clean_nan(c) for d, c in zip(df_pmc['date'], df_pmc['CTL'])}
        pmc_map_atl = {d: clean_nan(a) for d, a in zip(df_pmc['date'], df_pmc['ATL'])}
        pmc_map_tsb = {d: clean_nan(t) for d, t in zip(df_pmc['date'], df_pmc['TSB'])}
        pmc_map_load = {d: clean_nan(l) for d, l in zip(df_pmc['date'], df_pmc['daily_friction_load'])}
