...
def build_overlay_data(summary_data):
    ...
    try:
        ...
        # Bolt: Replaced df.iterrows() with zip() for better performance iterating over Pandas DataFrame
        pmc_map_ctl = {date: clean_nan(ctl) for date, ctl in zip(df_pmc['date'], df_pmc['CTL'])}
        pmc_map_atl = {date: clean_nan(atl) for date, atl in zip(df_pmc['date'], df_pmc['ATL'])}
        pmc_map_tsb = {date: clean_nan(tsb) for date, tsb in zip(df_pmc['date'], df_pmc['TSB'])}
        pmc_map_load = {date: clean_nan(load) for date, load in zip(df_pmc['date'], df_pmc['daily_friction_load'])}
