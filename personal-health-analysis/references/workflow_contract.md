...
- 本机部署记录（2026-09-18）：技能分析/认证解释器为 `C:/Users/shich/.pi/agent/skills/personal-health-analysis/.venv/Scripts/python.exe`（按 `requirements.lock.txt` 离线 wheelhouse 安装，含 `garminconnect==0.3.16`），同步 runner 为 `C:/Users/shich/.pi/agent/venvs/personal-health-sync-390/Scripts/python.exe`（`garmindb==3.9.0` + `garminconnect==0.3.16`，以 `--no-deps` 安装 garmindb 再单独钉 garminconnect）；`…/personal-health-sync`（garmindb 3.8.0）保留为回滚 runner，全局解释器未升级。同步命令必须显式传入 3.9.0 runner 作为 `--garmindb-python`。
