...
显式调用本技能即授权读取请求窗口内相关健康指标，未给窗口时使用技能交互默认最近 14 天（N=14）；用户指定更窄指标时必须缩小。本地明确 `no_data` 后，技能可对同一窗口和组件自动消费一次实时只读授权，但执行前仍须说明将使用非官方 Garmin Connect 接口，并让当前选定解释器通过 `<SKILL_PYTHON> scripts/runtime_preflight.py --mode live`。该默认授权不得推导登录、认证状态探测、刷新令牌、同步、下载轨迹或扩大日期范围；这些动作仍需另行明确授权。
