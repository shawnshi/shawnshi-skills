...
- **Telemetry 落盘**: 任务结束时，必须使用 `write_to_file` 将元数据以 JSON 格式保存至隔离沙盒：`<appDataDir>\brain\<conversation-id>\scratch\telemetry.json`，彻底规避高频并发死锁。
- **交付链接契约**: 无论生成的是日常分析简报还是 HTML 大屏，主代理最后必须通过聊天框向用户输出可点击的 Markdown 链接（例如：`[查看生物态势看板](file:///C:/Users/shich/...)` 或指向 scratch 目录中生成的任何 Markdown 报告）。
