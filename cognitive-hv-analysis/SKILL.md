...
AVOID: 禁止只有时间线无横向对比；禁止把单一观点包装成结论；严禁在报告中遗漏重要实体的双链图谱标记；严禁越界将原始抓取数据写入核心图谱。
</strategy-gene>
...
### 并行搜索策略 (Sub-agent Orchestration)

必须使用原生的 `invoke_subagent` 工具，一次性唤醒多个 `research` 子代理进行并行搜索，以对抗长文本降智和提升榨取效率。建议的分工：
...
> - 使用 `search_web` 工具来发现信息来源和线索。多次搜索、多个关键词组合，不要只搜一次就放弃。
> - 找到具体网址后，直接使用 `read_url_content` 工具提取页面的纯净 Markdown 内容。
> - 一手来源优于二手来源：官方博客 > 权威媒体原创报道 > 转载/聚合
> - **学术类对象必查 arxiv**：如果研究对象涉及学术概念、算法、AI模型、技术范式等，必须获取相关论文。无需使用 curl，直接使用 `read_url_content` 工具访问 API 链接提取数据：`https://export.arxiv.org/api/query?search_query=all:关键词1+AND+all:关键词2&max_results=10`。返回 XML 格式，找到关键论文后，再次调用 `read_url_content` 读取论文页面。
...
## 6. <Contracts> (输出与交付契约)
- **文档完整度门禁**：交付的 PDF 研究报告总字数必须在 10,000-30,000 字的范围内，且完全匹配纵向分析、横向竞品图谱、横纵交汇洞察的三段式结构。
- **强制图谱双链**：对报告中出现的核心企业、人物或专有名词，必须使用 `[[ ]]` 进行硬链接以便于图谱引擎捕获。
- **Telemetry 记录**: 任务结束时，必须使用 `write_to_file` 工具将元数据保存至绝对路径 `C:\Users\shich\.gemini\MEMORY\skill_audit\telemetry\record_[TIMESTAMP].json`。
  JSON 结构：`{"skill_name": "cognitive-hv-analysis", "status": "success", "duration_sec": 0, "input_tokens": 0, "output_tokens": 0}`

## 7. <Failure_Taxonomy> (失败分类学)
- **幻觉工具调用**：严禁伪造 `WebSearch`、`WebFetch`。必须使用原生支持的 `search_web` 和 `read_url_content`。
- **非并发搜索 (Sequential Trap)**：如果分析对象极其复杂，严禁主代理自行一步步搜索导致 Context 耗尽，必须通过 `invoke_subagent` 派发并行研究任务。
- **内容注水与机械罗列**：禁止把单一事实包装成洞察；禁止将发展历程写成毫无因果关系的年表流水账；禁止只有时间线梳理而没有实质性的同期竞品横向对比。
- **路径漂移**：严禁把报告保存至临时文件夹，所有分析报告必须锚定于 `C:\Users\shich\.gemini\MEMORY\raw\research` 目录。
- **虚假自信 (False Certainty)**：如果在网络中实在无法搜索到某个维度的关键事实，必须诚实标注「该信息暂缺」，绝不允许通过推理强行编造。
