...
version: 9.0.0
tier: action-allowed
description: '横纵深度研究引擎。执行纵向演化追踪与横向同期比较。产出结构化长篇研报。禁止缺少横向对比，禁止流水账编年史，禁止不验证强行编造。'
---
...
Strategy:
1. 锚定对象：确认时间轴、同类参照物和评价维度。
2. 双轨追踪：纵向追踪关键阶段，横向比较同期替代方案和竞争格局。
3. 产出交付：输出结构化判断、证据链、非共识点和风险边界。
4. 语义双链：对核心实体使用 `[[ ]]` 进行硬链接，遵循 Compiled Truth 规范。
AVOID: 仅有时间线无横向对比；单一观点包装成结论；遗漏双链图谱标记；自行越界修改底层图谱。
</strategy-gene>

# Cognitive HV Analysis (横纵分析法深度研究 V9.0)

> **方法论溯源**: 横纵分析法融合历时-共时分析、纵向-横截面研究设计、商学院案例研究法，形成一套适用于产品/公司/概念/人物的研究框架。纵向追时间深度，横向追同期广度。

## Tool Trajectory
**[IN_ORDER]** 执行需遵循以下轨迹流：
1. `invoke_subagent` (强制派发并发子代理执行资料搜寻)
2. `write_to_file` (主代理汇总子代理情报，落盘 Markdown 长篇战报)

## 0. 前置准备 (Context Setup)
拿到用户输入后，确认以下信息（若足够明确则直接开始）：
1. **研究对象**：产品/公司/概念/人名
2. **研究动机**：近期事件或核心切入点
3. **特别关注点**：重点深入方向

## 1. 联网信息收集 (Parallel Orchestration)
必须使用原生的 `invoke_subagent` 工具，一次性唤醒多个 `research` 子代理进行并行搜索。
...
**子Agent联网工具使用指南**（直接写入发送给子代理的 Prompt 中）：
> "你需要联网获取深度信息，请严格使用系统原生工具：
> - 使用 `search_web` 发现信息来源。
> - 使用 `read_url_content` 提取页面纯净 Markdown 内容。
> - 一手来源优于二手来源（官方 > 权威原创 > 转载）。
> - 若涉及学术概念，必须使用 `read_url_content` 访问 `https://export.arxiv.org/api/query?search_query=all:关键词&max_results=10` 获取 XML 并读取核心论文。"

为主代理下达指令时要描述明确的研究目标，子代理将利用自身沙盒工具自主判断最佳获取方式。
...
文件命名为 `[研究对象名称]_横纵分析报告_[date].md`，使用 `write_to_file` 保存到绝对物理路径 `C:\Users\shich\.gemini\MEMORY\raw\research` 目录下。
...
## 2. <Contracts> (输出与交付契约)
- **文档完整度门禁**：交付的研究报告必须完全匹配纵向分析、横向竞品图谱、横纵交汇洞察的三段式结构。字数通常在万字级别。
- **强制图谱双链**：报告中出现的核心实体，必须使用 `[[ ]]` 进行硬链接以便于图谱引擎捕获。
- **物理落盘要求**：最终报告必须使用 `write_to_file` 保存至绝对路径 `C:\Users\shich\.gemini\MEMORY\raw\research\[研究对象名称]_横纵分析报告_[date].md`。
- **Telemetry 记录**: 任务结束时，使用 `write_to_file` 将元数据保存至：
  `C:\Users\shich\.gemini\MEMORY\skill_audit\telemetry\record_[TIMESTAMP].json`

## 3. <Failure_Taxonomy> (失败分类学)
- **非并发搜索 (Sequential Trap)**：对于极其复杂的分析对象，主代理自行一步步搜索导致 Context 耗尽，未使用 `invoke_subagent` 并发。
- **内容注水与机械罗列**：把单一事实包装成洞察；将发展历程写成毫无因果关系的年表流水账；只有时间线而无横向对比。
- **虚假自信 (False Certainty)**：如果在网络中无法搜索到某个维度的关键事实，通过推理强行编造，而非诚实标注“该信息暂缺”。
- **工具越权**: 伪造不存在的 API 爬取数据，未使用原生的 `search_web` 和 `read_url_content`。
