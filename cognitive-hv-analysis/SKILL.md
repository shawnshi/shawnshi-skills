...
> - 如果当前运行时提供浏览器级访问能力，优先使用该能力执行重度页面提取与交互
...
文件命名为 `[研究对象名称]_横纵分析报告._[date].md`，保存到`{user_root}\.gemini\MEMORY\raw\research。
...
## When to Use
- 当用户要求对产品、公司、概念、技术或人物做系统性横纵分析时使用。
- 纵向生命历程、横向对比和最终交叉洞察仍以本文件既有协议为准。

## Workflow
- 遵循本文件已有的纵轴叙事、横轴竞品比较、交叉分析和成稿流程。
- 不跳过时间线梳理、横向同类比较和最终 PDF 研究报告要求。

## Resources
- 使用本技能引用的模板、脚本、参考框架、排版资源和图表资产。
- 任何对比维度、报告样式和输出路径以技能目录现有资源为准。

## Failure Modes
- 将本文件中的分析框架、排版清单和 `NLAH Gotchas` 视为失败模式。
- 若研究对象、对比范围或输出格式不清晰，必须先界定研究边界。

## Output Contract
- 最终交付必须包含纵向故事线、横向对比表/结构，以及交叉得到的独到洞察。
- 研究报告必须满足本文件要求的结构完整性与最终产物质量门槛。
