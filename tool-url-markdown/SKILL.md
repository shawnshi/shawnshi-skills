...
<strategy-gene>
Keywords: URL 转 Markdown, 网页总结, 保存网页, JS 渲染
Summary: 抽取网页主体内容并转换为干净 Markdown。
Strategy:
1. 判断网页是否静态、动态渲染或需要浏览器提取。
2. 清理导航、广告、脚本噪音并保留正文结构。
3. 输出 Markdown 路径或正文，并标明提取限制。
AVOID: 禁止把抓取失败内容当正文；禁止保留广告导航噪音。
</strategy-gene>
...
```bash
npx -y bun {root_dir}\.gemini\skills\tool-url-markdown\scripts\main.ts "<URL>"
```

### 2. Login-Gated Capture (Private Pages)
For Twitter, Substack, or internal dashboards:
```bash
npx -y bun {root_dir}\.gemini\skills\tool-url-markdown\scripts\main.ts "<URL>" --wait
```
**Protocol**:
...
```bash
npx -y bun {root_dir}\.gemini\skills\tool-url-markdown\scripts\main.ts "<URL>" -o "docs/article.md"
```
...
- JSON 结构：`{"skill_name": "tool-url-markdown", "status": "success", "duration_sec": [ESTIMATE], "input_tokens": [ESTIMATE], "output_tokens": [ESTIMATE]}`
