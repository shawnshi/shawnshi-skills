...
生成/重构及严格流水线共同必需输入：来源与敏感级别、科学问题、投稿阶段、目标期刊或 generic-draft、图件类型、全部展示文本、字体、文件名。

## 成品审查输入与认证边界

有限成品审查先用最终文件和已知用途；图注、目标尺寸/期刊、来源与敏感级别不足时，只询问会改变安全处理或审查结论的缺口。未核验的字体、统计证据、文本或期刊属性记 NOT_CHECKED，不要求为有限结果重建生成 Job；敏感处理门不因缺信息而豁免。必须在可用能力内渲染/生成预览并进行真实视觉复核，未完成检查只列限制，保持 draft，不授予 visually_checked 或 journal_verified。

以下 Job、visual_review、preview/final 与标准交付包属于既有严格流水线契约，包括完整 visual_audit；未新增业务 mode 或放宽 Schema。完整认证仍要求全部必需输入、物理视觉检查及成品检查通过。有限审查报告不得冒充严格流水线回执。
