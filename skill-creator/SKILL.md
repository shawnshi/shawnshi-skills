...
- **MSL 原子化**: 业务逻辑 Skill 化。
- **Schema 绝对防御**: [Template] 100% 同态。

### C. 执行层：负熵交互与 OODA 闭环 (Execution)
- **脑暴倾倒 + 查漏补缺**: 废除串行审讯。
- **Markdown 原生可视化**: 强制使用 Mermaid。

### D. 进化层：自愈能力与证据网 (Evolution)
- **失效先验 (Gotchas)**: 禁令回写。
- **证据网 (Evidence-Mesh)**: 物理归档至 `MEMORY/skill_audit/`。
...
统计基准得分。若基准得分满分，拒绝优化。
Decide (提出突变假设):
基于失败用例，提出且仅提出一个针对性的改动假设（例如：添加具体的反向模式 Anti-pattern）。
Act (突变、复测与资产化):
