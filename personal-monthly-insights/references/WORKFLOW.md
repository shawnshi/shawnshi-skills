...
1. **Analyze Metrics**: Read the generated `~/.gemini/MEMORY/raw/personal-insights/raw_metrics_<PERIOD>.json`.
...
1. **Save Reasoning**: Follow the strict structure in [SCHEMA.md](SCHEMA.md) and use `write_file` to save to `~/.gemini/MEMORY/raw/personal-insights/agent_audit_result.json`.
2. **Formatting**: Ensure `coach_summary` is qualitative and `prompt_assets` are ready-to-copy.

## Stage 4: Render & Deliver
1. **Generate Dashboard**: Run `python analyze_insights_v4.py --period <PERIOD> --render --agent-file ~/.gemini/MEMORY/raw/personal-insights/agent_audit_result.json`.
