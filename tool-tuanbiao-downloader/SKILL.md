...
```bash
# 若报错，请安装
pip install -r {root_dir}\.gemini\skills\tool-tuanbiao-downloader\scripts\requirements.txt
```

### 2. Execution (执行)
直接提供 **Path ID** 或 **预览 URL**：
```bash
python {root_dir}\.gemini\skills\tool-tuanbiao-downloader\scripts\downloader.py <ID_OR_URL>
```
...
*   **内容理解**: 调用 `${tool-document-summarizer}` 为生成的 PDF 生成摘要。
