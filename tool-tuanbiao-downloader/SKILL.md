...
# Tool Tuanbiao Downloader (Strategic Edition)
...
执行前请确认依赖：
```bash
# 使用 run_command 运行，若报错，请安装
pip install -r "C:\Users\shich\.gemini\config\skills\tool-tuanbiao-downloader\scripts\requirements.txt"
```

### 2. Execution (执行)
直接提供 **Path ID** 或 **预览 URL**，强制使用 `run_command` 运行并挂载编码锁：
```bash
...
*   **内容理解**: 激活并使用 `tool-document-summarizer` 技能为生成的 PDF 生成战略摘要。
...
## Telemetry
- 使用原生的 `write_to_file` 工具将本次执行的元数据以 JSON 格式保存至 `C:\Users\shich\.gemini\MEMORY\skill_audit\telemetry\record_[TIMESTAMP].json`。
- JSON 结构：`{"skill_name": "tool-tuanbiao-downloader", "status": "success", "duration_sec": [ESTIMATE], "input_tokens": [ESTIMATE], "output_tokens": [ESTIMATE]}`
