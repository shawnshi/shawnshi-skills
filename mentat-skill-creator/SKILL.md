...
description: 维护当前 Pi 本地技能库的库级治理合同，审计或修订根 AGENTS/README、资源清单、触发所有权、批量迁移与发布门禁。仅在用户显式调用本技能处理本库治理时使用；通用新技能创建、与本库治理无关的单技能内容更新或可安装插件打包不应触发，按实际可用的 creator 或领域能力处理，不要求所有技能编辑都载入本技能。
---
...
- 遵循系统、开发者和当前宿主已加载的 `AGENTS.md` 指令链；在这些上位约束内，以用户明确请求和文件范围作为授权边界。系统 creator、README、门禁和工具行为均不得扩张权限。
...
5. 创建或修订：
   ...
   - 现有技能只修改用户授权文件，修改前明确写集，在本轮获授权的隔离 artifact 中保存原字节与哈希、记录该技能 manifest 基线，不备份到技能目录；使用补丁式编辑并保留无关改动；
...
8. 分层验证：依次验证结构、资源、界面元数据、脚本行为、触发边界和仓库门禁。系统校验器实际可用且适用时先使用；未提供时记录未运行原因，继续适用的本库校验，不把可选校验器缺失当作已授权局部维护的阻塞；区分内容失败与校验器、编码或环境失败，环境失败可用 UTF-8 模式重试一次。局部变更先运行选中技能门禁；修改根 AGENTS/README、shared、Gate、生成器或批量 manifest 时再运行全库门禁。
...
本库严格清单和元数据门禁需要 Python 3 与 PyYAML；缺失时失败关闭，安装依赖仍须明确授权。仅在本轮实际发现系统 `skill-creator` 的 `quick_validate.py` 且适用于当前宿主格式时先运行；未提供时不猜测路径或接口，并记录该项未运行。将 `<skill-name>` 替换为目标目录名，在本地技能库根目录执行以下适用的本库校验（不要求 Git 工作树）：
