...
# HIT Industry Radar (医疗行业雷达 V8.1 Native)
...
3. **静默挂起与硬锁读取**: 主代理必须结束当前对话轮次，静默等待所有子代理向临时文件 `C:\Users\shich\.gemini\tmp\raw_scout_data_[战区].json` 写入底层抓取数据。主代理恢复时，必须使用 `view_file` 验证这些数据确实存在。
...
1. 根据 <Contracts> 要求的 `[Format Stack]` 渲染草稿，强制写入临时文件 `C:\Users\shich\.gemini\tmp\draft_hit_radar.md`。
2. **强制过检**: 调用原生的 `run_command` 执行跨平台审计脚本（必须挂载 UTF-8 数据流锁并使用绝对物理路径）：
   `$env:PYTHONIOENCODING="utf-8"; python "C:\Users\shich\.gemini\config\skills\hit-industry-radar\scripts\hit_audit_gate.py" "C:\Users\shich\.gemini\tmp\draft_hit_radar.md" --mode radar`
...
- **路径与工具死锁 (Pathing Deadlock)**：严禁写入漏层级的脚本路径。必须执行绝对物理寻址 `C:\Users\shich\.gemini\config\skills\hit-industry-radar\scripts\hit_audit_gate.py`。落盘工具必须使用原生的 `write_to_file`。
