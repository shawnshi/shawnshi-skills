...
2. 检索前读 [references/research_contract.md](references/research_contract.md)：有界检索、记录覆盖，在现有上限内预留高影响事实与独立血缘核验额度，先减少低价值候选，不挤占核验后再自动扩额；区分事件/发布日期、背景/未知日期线索，按项目/标段/阶段去重，转载不算独立证据。预算仅为规划界限，不保证运行时强制执行。
