...
2. **五维清洗**: 剔除留存 Fact 中的所有形容词与公关废话（严禁“赋能”、“生态”、“智慧体系”等空话），仅留时间/金额/版本/核心临床KPI（如DRG控费、评级过检）。
...
### Phase 4: 分层归档与图谱入湖 (The Hard Gate)
1. **物理落盘**: 质检通过后，使用 `write_to_file` 写入：
   `C:\Users\shich\.gemini\MEMORY\raw\HealthcareIndustryRadar\DHWB-Radar-YYYYMMDD.md`
2. **异步入湖 (STQM & Payload MCP)**: 提取含双链 `[[ ]]` 的战略突变与价格战冲突，强制调用 `invoke_subagent` 拉起入湖子代理。
   - 子代理指令必须明确：“将情报提纯并使用 `write_to_file` 写入 `scratch/ingest_payload.json`，如有行业冲突必须提取为 `tension_edges`。最后调用 `vector-lake-mcp:prepare_ingest_batch` 执行入湖，绝对禁止通过命令行参数传递长文本。”
