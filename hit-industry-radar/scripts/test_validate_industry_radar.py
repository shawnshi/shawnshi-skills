...
def fixture(start="2026-09-04", end="2026-09-10", cutoff="2026-09-10T09:00:00+08:00",
            mode="rolling7", status="complete", events=True, zone="Asia/Shanghai"):
    # Tests deliberately mutate typed API arguments, including invalid values.
    args: dict[str, Any] = {"period_start": date.fromisoformat(start), "period_end": date.fromisoformat(end),
                            "issue_date": date.fromisoformat(end), "cutoff": datetime.fromisoformat(cutoff),
                            "window_mode": mode, "report_timezone": zone, "now": NOW}
    rows = [["E1", end, end, "合成医院", "公布采购结果", "合成医院/A/包1/中标", "E1", "S1", "可能签约", "合同未披露"]] if events else []
    source_rows = [["S1", "https://example.org/award", "L1", "primary", "accessed"]] if events else []
    marker = "" if events else "\n" + {"complete": EMPTY, "partial": PARTIAL_EMPTY, "blocked": BLOCKED_EMPTY}[status]
...
class ValidateIndustryRadarTests(unittest.TestCase):
    ...
    def test_valid_default_includes_today(self):
        ...
        self.assertEqual([], self.check(self.content.replace("| 2026-09-10 | 2026-09-10 |", "| 2026-09-04 | 2026-09-10 |")))
    ...
    def test_natural_week_and_monday_single_day(self):
        for start, end in [("2026-09-07", "2026-09-10"), ("2026-09-07", "2026-09-07")]:
            content, args = fixture(start, end, end + "T09:00:00+08:00", mode="natural_week")
            self.assertEqual([], self.check(content, args))
    ...
    def test_aware_cutoff_now_and_future(self):
        for change in ({"cutoff": datetime(2026, 9, 10, 9)}, {"now": datetime(2026, 9, 11)},
    ...
    def test_timezone_conversion_not_timestamp_date(self):
        ...
        self.assertEqual([], self.check(content, args))
        self.reject(args={**self.args, "report_timezone": "Not/AZone"}, contains="timezone")

    def test_event_date_invalid_unknown_outside(self):
        for day in ("2026-02-30", "2026-09-03", "2026-09-11", "未知", "20260910", "2026-9-10"):
            with self.subTest(day=day):
                self.reject(self.content.replace("| 2026-09-10 | 2026-09-10 |", f"| {day} | 2026-09-10 |"))

    def test_publication_can_precede_event_or_be_unknown(self):
        for published in ("2026-08-01", "未知"):
            self.assertEqual([], self.check(self.content.replace("| 2026-09-10 | 2026-09-10 |", f"| 2026-09-10 | {published} |")))

    def test_publication_invalid_or_future(self):
        for published in ("2026-09-11", "2026-02-30", "20260910"):
            self.reject(self.content.replace("| 2026-09-10 | 2026-09-10 |", f"| 2026-09-10 | {published} |"))
    ...
    def test_strict_utf8_bom_replacement_and_empty(self):
        for data in (b"\xff", b"", ("\ufeff" + self.content).encode(), (self.content + "\ufffd").encode()):
            self.reject(data)
    ...
    def test_header_uniqueness_missing_mismatch(self):
        ...
        self.reject(self.content.replace("生成时点：2026-09-10T09:00:00+08:00", "生成时点：2026-09-10T09:01:00+08:00"))
    ...
    def test_required_metadata_rejects_blank_values(self):
        for line in self.content.splitlines()[1:8]:
            ...
            for blank in ("", "   ", "\t\t", "\u3000\u3000"):
                for newline in ("\n", "\r\n"):
                    with self.subTest(label=label, blank=repr(blank), newline=repr(newline)):
                        content = self.content.replace(line, label + "：" + blank, 1).replace("\n", newline)
                        with self.assertRaisesRegex(ValueError, label + " must not be blank"):
        ...
        for line in self.content.splitlines()[1:8]:
            ...
            for blank in ("", " \t\u3000"):
                ...
                for first in (True, False):
                    ...
                    for newline in ("\n", "\r\n"):
                        with self.subTest(label=label, blank=repr(blank), first=first, newline=repr(newline)):
                            content = self.content.replace(line, replacement, 1).replace("\n", newline)
                            with self.assertRaisesRegex(ValueError, label + " must occur exactly once"):
                                metadata(content, label)
                            self.reject(content, contains=label + " must occur exactly once")
    ...
    def test_required_metadata_strips_surrounding_whitespace(self):
        ...
        for line in self.content.splitlines()[1:8]:
            label, value = line.split("：", 1)
            content = content.replace(line, label + "： \t\u3000" + value + "\u3000\t ", 1)
        self.assertEqual([], self.check(content))
    ...
    def test_filename_issue_title_and_custom_filename(self):
        ...
        self.assertEqual([], self.check(args={**self.args, "allow_custom_filename": True}, name="custom.md"))
        self.reject(args={**self.args, "issue_date": date(2026, 9, 9)}, contains="issue date")
        self.reject(self.content.replace("# 医疗行业雷达", "# 数字健康周报", 1), contains="title")
    ...
    def test_url_syntax(self):
        ...
        for url in ("file:///x", "javascript:alert(1)", "https://", "https://a:99999/x", "https://a:no/x",
                    "https://user:pass@example.org/x", "https://example.org/ 注释", "[x](https://example.org)",
                    "https://[broken", "https://example.org/\n", "https://example.org/\\x"):
    ...
    def test_duplicate_event_ids_and_keys(self):
        row = next(line for line in self.content.splitlines() if line.startswith("| E1 |"))
        duplicate_id = row.replace("/包1/", "/包2/").replace("公布采购结果", "公布合同")
        self.reject(self.content.replace(row, row + "\n" + duplicate_id), contains="duplicate event ID")
        for duplicate in (row.replace("| E1 |", "| E2 |", 1),
                          row.replace("| E1 |", "| E2 |", 1).replace("公布采购结果", "结果公布")
                          .replace("合成医院/A/包1/中标", " 合成医院/ a /包1/中标 ")):
            self.reject(self.content.replace(row, row + "\n" + duplicate), contains="duplicate event key")

    def test_distinct_procurement_projects_and_lots_same_action_allowed(self):
        row = next(line for line in self.content.splitlines() if line.startswith("| E1 |"))
        for key in ("合成医院/A/包2/中标", "合成医院/B/包1/中标"):
            with self.subTest(key=key):
                extra = row.replace("| E1 |", "| E2 |", 1).replace("合成医院/A/包1/中标", key)
                self.assertEqual([], self.check(self.content.replace(row, row + "\n" + extra)))

    def test_distinct_procurement_stage_allowed(self):
        row = next(line for line in self.content.splitlines() if line.startswith("| E1 |"))
        contract = row.replace("| E1 |", "| E2 |", 1).replace("公布采购结果", "公布合同").replace("/中标", "/合同")
        self.assertEqual([], self.check(self.content.replace(row, row + "\n" + contract)))

    def test_source_references_and_duplicate_sources(self):
        self.reject(self.content.replace("| E1 | S1 |", "| E1 | S9 |"), contains="unknown source")
        self.reject(self.content.replace("| E1 | S1 |", "| E1 | S1,S1 |"), contains="duplicate source references")
        row = "| S1 | https://example.org/award | L1 | primary | accessed |"
        self.reject(self.content.replace(row, row + "\n" + row), contains="duplicate source")
        self.reject(self.content.replace(row, row + "\n" + row.replace("S1", "S2").replace("/award", "/award#copy")), contains="duplicate source URL")
    ...
    def test_source_url_case_and_fragment_duplicates_rejected(self):
        ...
        for url in ("https://EXAMPLE.ORG/award", "HTTPS://example.org/award",
                    "HTTPS://EXAMPLE.ORG/award#copy"):
            with self.subTest(url=url):
                extra = f"| S2 | {url} | L2 | primary | accessed |"
                self.reject(strong.replace(row, row + "\n" + extra), contains="duplicate source URL")

    def test_source_url_path_query_and_port_distinctions_preserved(self):
        row = "| S1 | https://example.org/award | L1 | primary | accessed |"
        for first, second in (("/award?id=A/", "/award?id=A"),
                              ("/award/", "/award"), ("/Award", "/award"),
                              ("/award?id=A", "/award?id=a"),
                              ("/award?id=A", "/award?id=B"),
                              (":8443/award", "/award")):
            with self.subTest(first=first, second=second):
                sources = (f"| S1 | https://example.org{first} | L1 | primary | accessed |\n"
    ...
    def test_legal_bracketed_ipv6_sources_allowed(self):
        for url in ("https://[2001:db8::1]/notice", "http://[::1]:8080/notice"):
            with self.subTest(url=url):
                self.assertTrue(original_link(url))
                self.assertEqual([], self.check(self.content.replace("https://example.org/award", url)))

    def test_malformed_bracketed_hosts_and_markdown_rejected(self):
        for url in ("https://[not-ipv6]/notice", "https://[2001:db8::1/notice",
                    "https://[2001:db8::1]extra/notice", "https://[::1]:bad/notice",
                    "[notice](https://[2001:db8::1]/notice)", "https://example.org/[notice]"):
            with self.subTest(url=url):
                self.assertFalse(original_link(url))
                self.reject(self.content.replace("https://example.org/award", url), contains="invalid original")

    def test_evidence_lineage_constraints(self):
        strong = self.content.replace("| E1 | S1 |", "| E2 | S1,S2 |")
        row = "| S1 | https://example.org/award | L1 | primary | accessed |"
        for lineage, kind, access, valid in [("L1", "secondary", "accessed", False),
                                           ("L2", "secondary", "unavailable", False),
                                           ("L2", "unverified", "accessed", False),
                                           ("L2", "secondary", "accessed", True)]:
            extra = f"| S2 | https://example.net/report | {lineage} | {kind} | {access} |"
            errors = self.check(strong.replace(row, row + "\n" + extra))
            self.assertEqual(valid, not errors, errors)
        self.reject(self.content.replace("primary | accessed", "primary | unavailable"), contains="direct source")
        self.reject(self.content.replace("primary | accessed", "secondary | accessed"), contains="direct source")
        self.reject(self.content.replace("| E1 | S1 |", "| lead | S1 |"), contains="evidence")
        self.assertEqual([], self.check(self.content.replace("primary | accessed", "marketing | accessed")))
        all_marketing = strong.replace(row, row + "\n| S2 | https://example.net/report | L2 | marketing | accessed |")
        self.reject(all_marketing.replace("primary | accessed", "marketing | accessed"), contains="E2 needs")
        ...
        for status in ("complete", "partial", "blocked"):
            with self.subTest(status=status):
                ...
                for marker in (EMPTY, PARTIAL_EMPTY, BLOCKED_EMPTY):
                    expected = {"complete": EMPTY, "partial": PARTIAL_EMPTY, "blocked": BLOCKED_EMPTY}[status]
                    if marker != expected:
    ...
    def test_empty_marker_cannot_coexist_with_events_or_repeat(self):
        self.reject(self.content.replace("## 关键事件", "## 关键事件\n" + EMPTY), contains="marker")
        content, args = fixture(events=False)
        self.reject(content.replace(EMPTY, EMPTY + "\n" + EMPTY), args, "marker")

    def test_coverage_status_consistency(self):
        self.reject(self.content.replace("| 采购 | complete |", "| 采购 | blocked |"), contains="coverage")
        self.reject(self.content.replace("检索状态：complete", "检索状态：partial"), contains="partial requires")
        self.reject(self.content.replace("检索状态：complete", "检索状态：blocked"), contains="blocked")
        content, args = fixture(status="partial")
        self.assertEqual([], self.check(content, args))
        self.reject(content.replace("| 采购 | partial |", "| 采购 | blocked |"), args, "partial requires")
    ...
    def test_required_sections_tables_cells_and_coverage(self):
        for title in ("关键事件", "来源", "检索覆盖", "结论摘要", "信息缺口"):
            ...
            self.reject(self.content + "\n## " + title + "\n重复")
        self.reject(self.content.replace("| 可能签约 |", "| |"), contains="empty table cell")
        self.reject(self.content.replace("| 事件ID |", "| ID |"), contains="header")
        self.reject(self.content.replace("## 关键事件", "## 关键事件\n绕过日期的自由事件"), contains="table rows")
        row = next(line for line in self.content.splitlines() if line.startswith("| 采购 |"))
    ...
    def test_assets_and_manual_eval_contracts(self):
        targets = json.loads((ROOT / "assets/intelligence_targets.json").read_text(encoding="utf-8"))
        self.assertEqual({"global_hit", "china_hit", "winning_baseline"}, set(targets["targets"]))
        for urls in targets["targets"].values():
            self.assertTrue(all(original_link(url) for url in urls))
        for filename, case_key in (("benchmark.json", "test_cases"), ("evals.json", "evals")):
    ...
    def test_cli_positive_negative_and_no_writes(self):
        ...
        cmd = [sys.executable, "-B", str(ROOT / "scripts/validate_industry_radar.py"),
               "--file", str(file), "--period-start", str(args["period_start"]),
               "--period-end", str(args["period_end"]), "--issue-date", str(args["issue_date"]),
               "--cutoff", args["cutoff"].isoformat()]
        for expected, data in [(0, content), (1, content.replace("| 2024-03-01 | 2024-03-01 |", "| 2024-03-02 | 2024-03-01 |"))]:
