...
EVENT_HEADER = ["事件ID", "事件日期", "发布日期", "主体", "已核实动作", "事件键", "证据强度", "来源编号", "影响（推断）", "限制"]
SOURCE_HEADER = ["来源编号", "原始链接", "血缘", "类型", "访问状态"]
COVERAGE_HEADER = ["检索面", "状态", "查询/来源及结果说明"]
HEADERS = {"## 关键事件": EVENT_HEADER, "## 来源": SOURCE_HEADER, "## 检索覆盖": COVERAGE_HEADER}
PLACEHOLDER_RE = re.compile(r"\[(?:PERIOD_START|PERIOD_END|ISSUE_DATE|CUTOFF_TIME[^\]]*|REPORT_TIMEZONE|WINDOW_MODE|待填写[^\]]*|YYYY-MM-DD[^\]]*)\]")
...
def original_link(value: str) -> bool:
    try:
        ...
        if ("[" in parsed.netloc or "]" in parsed.netloc) and not re.fullmatch(r"\[[^\[\]]+\](?::[0-9]+)?", parsed.netloc):
            return False
...
def validate_report(file_path: Path, period_start: date, period_end: date,
                    issue_date: date, cutoff: datetime, window_mode: str = "rolling7",
    ...
    if window_mode == "rolling7":
        ...
        if period_end != cutoff_date or period_start != cutoff_date - timedelta(days=cutoff_date.weekday()):
            errors.append("natural_week must run Monday through cutoff date")
    elif window_mode != "explicit":
        errors.append("unknown window mode")
    if not allow_custom_filename and file_path.name != f"DHWB-Radar-{issue_date:%Y%m%d}.md":
    ...
    try:
        expected = {"报告周期": f"{period_start} 至 {period_end}", "出刊日期": str(issue_date),
                    "报告时区": report_timezone, "窗口模式": window_mode}
        for label, value in expected.items():
            if metadata(content, label) != value:
        ...
        metadata(content, "报告范围")
        status = metadata(content, "检索状态")
    ...
    for number, raw in enumerate(lines, 1):
        ...
        if cells == HEADERS[section]:
            ...
            continue
        if len(cells) == len(HEADERS[section]) and all(re.fullmatch(r":?-{3,}:?", c) for c in cells):
            continue
    ...
    for sid, url, lineage, kind, access in tables["## 来源"]:
        ...
        if not original_link(url):
            ...
            url_key = (parsed.scheme.lower(), parsed.netloc.lower(), parsed.path, parsed.query)
            if url_key in urls:
                errors.append("duplicate source URL")
            urls.add(url_key)
        if not re.fullmatch(r"L\d+", lineage):
            errors.append("invalid source lineage ID")
        if kind not in {"primary", "secondary", "marketing", "unverified"} or access not in {"accessed", "unavailable"}:
    ...
    keys: set[str] = set()
    events = tables["## 关键事件"]
    for rid, happened, published, _subject, _action, key, evidence, refs, _impact, _limits in events:
        ...
        try:
            ...
            errors.append("invalid event/publication date; unknown event belongs in leads")
        if evidence not in {"E1", "E2"}:
            errors.append("event evidence must be E1 or E2")
        ref_ids = refs.split(",")
        if not re.fullmatch(r"S\d+(?:,S\d+)*", refs) or len(set(ref_ids)) != len(ref_ids):
        ...
        usable = [sources[sid] for sid in ref_ids if sid in sources and sources[sid][3] == "accessed" and sources[sid][2] != "unverified"]
        if not any(source[2] in {"primary", "marketing"} for source in usable):
            errors.append("event needs an accessed direct source")
        if evidence == "E2" and (len({source[1] for source in usable}) < 2 or not any(source[2] != "marketing" for source in usable)):
            errors.append("E2 needs independent usable lineages, not marketing alone")
    expected_marker = {"complete": EMPTY, "partial": PARTIAL_EMPTY, "blocked": BLOCKED_EMPTY}.get(status)
    if (events and markers) or (not events and markers != [expected_marker]):
        errors.append("empty/event marker missing or contradictory")
    if status == "blocked" and (events or any(s[3] == "accessed" for s in sources.values())):
    ...
    if status == "partial" and (not any(state in {"complete", "partial"} for state in states) or all(state == "complete" for state in states)):
    ...
    if any(ref not in sources for ref in re.findall(r"\[(S\d+)\]", content)):
        errors.append("unknown narrative source reference ID")
    if any(ref not in ids for ref in re.findall(r"\[(E\d+)\]", content)):
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--window-mode", choices=("rolling7", "natural_week", "explicit"), default="rolling7")
    ...
    for error in errors:
        print("FAIL:", error)
    if not errors:
        print("OK: structural checks only; facts, lineage, source access and archive safety not verified")
