...
- **定点诊断**：单一连接器、一次调用失败或权限阻塞只分析该问题与必要事件，不强制展开完整四象限、六个结果问题或八个视角。仍遵守下述证据、计算与授权门；无证据时报告未知，不补造历史或自动采样。
- **综合证据化复盘**：只分析授权范围内的既有记录，不因读取技能而运行 Token 计量或写新遥测。缺少既有计量证据时报告缺口／不可计算；本轮补写回执不得充当历史事实。
...
4. 证据检查通过后，定点诊断仅围绕事件序列、原因或未知及最小修复分析；综合复盘才读取 [references/ANALYSIS.md](references/ANALYSIS.md)，先看任务结局、用户介入／返工与上次整改后续，再展开有证据且相关的分析视角，不凑齐八个视角。仅消费显式注释与已授权来源；缺证据就标记不可用，不从消息数推断动机。等待、载入、重试、子代理、授权和压缩恢复按 WORKFLOW 作为解释证据，不以统计图数量代替任务结果。
...
**定点诊断**：问题与范围 → 事件证据 → 原因或未知及替代解释 → 最小修复建议 → 验证/停止条件。涉及指标仍列分子、分母、覆盖与不可计算项；不为窄问题加载完整报告模板，不默认保存或实施修复。

**综合复盘**：先读 [结果模板](references/report-template.md)，按适用证据组织以下结构：

1. 六个结果问题：已完成／未解决任务、代价最高的已证实可避免摩擦、AI 转交用户的负担、应保留的人类决策、上次整改后续、下一项唯一实验与证伪条件。
2. 各结论旁列证据、来源声明／核验状态、任务／介入／返工／cohort 分母、覆盖与未知。
3. 相关视角的发现、稳定 F-xx／R-xx 连接、定性／计算边界与替代解释。
