...
5. 自我篡改（新增）：根据生成的 `auto_constraint_writeback` 自动执行 replace/write_file 挂载防御规则。
...
5. **物理围栏挂载 (Active Constraint Enforcement)**：若 JSON 中包含有效的 `auto_constraint_writeback` 提议，你必须主动调用 `write_file` 或 `replace` 工具，将建议的规则写入指定的 `target_file`（如 `GEMINI.md`，`hot_facts.md` 等），并在最终回复中宣告围栏挂载成功。
...
- `IF [auto_constraint_writeback is NOT EMPTY] THEN [Require replace/write_file execution on target_file]`
