...
3. **基因解码**: 根据输出数据（含 Friction_Type 判定），生成“核心协作摩擦模式”、“教练解读”和“工作流资产”的 JSON 数据包，并使用 `write_to_file` 落盘到沙盒目录，例如 `C:\Users\shich\.gemini\MEMORY\scratch\mentat_audit_draft.json`。
4. **验证与渲染**: 必须依次运行：
   ```powershell
   $env:PYTHONIOENCODING="utf-8"; python "C:\Users\shich\.gemini\config\skills\mentat-collaboration-audit\validate_agent_audit.py" "C:\Users\shich\.gemini\MEMORY\scratch\mentat_audit_draft.json"
   $env:PYTHONIOENCODING="utf-8"; python "C:\Users\shich\.gemini\config\skills\mentat-collaboration-audit\analyze_insights_v4.py" --period <PERIOD> --render --agent-file "C:\Users\shich\.gemini\MEMORY\scratch\mentat_audit_draft.json"
   ```
5. **物理围栏挂载 (Active Constraint Enforcement)**：若 JSON 中包含有效的 `auto_constraint_writeback` 提议，你必须主动调用 `write_to_file` 工具，将建议的规则安全地写入指定的 `target_file`（如 `GEMINI.md` 或 `pai/` 文件），并在最终回复中宣告挂载成功。
...
- **Telemetry 元数据录入**: 报告落盘后，使用 `write_to_file` 将执行状态写入：
...
- **路径死锁 (Pathing Deadlock)**：严禁使用相对路径 `../` 或缺少 `config\` 层级的错误路径。所有的工具调用、脚本执行和写文件动作，**必须采用绝对物理路径**。严禁将 JSON 临时文件写在技能目录，必须写在 `MEMORY/scratch/`。
