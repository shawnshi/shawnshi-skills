...
5. （可选）执行自我篡改：根据 `auto_constraint_writeback` 主动调用 `write_file` 挂载系统防御围栏。
AVOID: 禁止没有数据依据的归因；禁止不输出具体行动指南的纯概念复盘；禁止使用虚伪客套的套话。
</strategy-gene>

# Mentat Collaboration Audit (联合审计管线 V11.1 Native)
...
1. **带锁调用探针**: 强制使用 `run_command` 调用外部脚本采集数据。
   ```powershell
   $env:PYTHONIOENCODING="utf-8"; python ../scripts/system_retro.py
   ```
2. **生成报告**: 输出包含 [全局算力损耗]、[异常节点狙击] 的冰冷数据报告。

### Mode 2: Interaction (深度协作模式)
**适用场景**：用户反映效率低下、协作阻力高，或要求完整的周/月度交互复盘。
1. **并行数据摄取**: 使用 `run_command` 并发或串行抓取底层与顶层数据：
   ```powershell
   $env:PYTHONIOENCODING="utf-8"; python analyze_insights_v4.py --period <PERIOD> --extract-only --drop-noise
   $env:PYTHONIOENCODING="utf-8"; python ../scripts/system_retro.py
   ```
2. **Schema 注入**: 必须使用 `view_file` 读取 `references/SCHEMA.md` 获取完整输出 JSON 格式模板。
3. **基因解码**: 根据输出数据（含 Friction_Type 判定），生成“核心协作摩擦模式”、“教练解读”和“工作流资产”的 JSON 数据包，并落盘到沙盒目录。
4. **验证与渲染**: 必须依次运行：
   ```powershell
   $env:PYTHONIOENCODING="utf-8"; python validate_agent_audit.py <json_path>
   $env:PYTHONIOENCODING="utf-8"; python analyze_insights_v4.py --period <PERIOD> --render --agent-file <json_path>
   ```
5. **物理围栏挂载 (Active Constraint Enforcement)**：若 JSON 中包含有效的 `auto_constraint_writeback` 提议，你必须主动调用 `write_file` 工具，将建议的规则安全地写入指定的 `target_file`（如 `GEMINI.md` 或 `pai/` 文件），并在最终回复中宣告挂载成功。

## 2. <Contracts> (输出与交付契约)

- **落盘报告**: 无论是 Mode 1 还是 Mode 2，最终的 Markdown 报告必须通过 `write_file` 物理落盘到审计目录：
  `C:\Users\shich\.gemini\MEMORY\skill_audit\audit_logs\mentat-collaboration-audit-[YYYY-MM-DD].md`。严禁仅仅在对话框中打出文本而不落盘。
- **Telemetry 元数据录入**: 报告落盘后，使用 `write_file` 将执行状态写入：
