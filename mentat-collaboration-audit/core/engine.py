...
def get_session_list():
    try:
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            shell=False, # SECURITY: Prevents command injection risk
            cwd=str(CLI_ROOT),
            encoding='utf-8',
            errors='ignore'
        )
...
def get_git_stats():
    ...
    try:
        ...
        return int(subprocess.check_output(cmd, encoding='utf-8', shell=False, cwd=str(git_root)).strip()) # SECURITY: Prevents command injection risk
    except Exception:
        return 0
