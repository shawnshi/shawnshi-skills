...
def get_session_list():
    try:
        # Security: shell=False prevents command injection when passing argument lists
        ...
        sessions = []
        brain_dir = DATA_ROOT / 'antigravity-cli' / 'brain'
        if brain_dir.exists():
            for sid_dir in brain_dir.iterdir():
                if sid_dir.is_dir() and re.match(r'^[a-f0-9]{8}-[a-f0-9]{4}', sid_dir.name):
                    date_str = datetime.date.today().isoformat()
                    try:
                        mtime = sid_dir.stat().st_mtime
                        date_str = datetime.datetime.fromtimestamp(mtime).date().isoformat()
                    except:
                        pass
                    sessions.append({'id': sid_dir.name, 'title': 'Session ' + sid_dir.name[:8], 'date': date_str})
        print(f'  🔍 从运行时根目录 fallback 提取到 {len(sessions)} 个会话记录')
        return sessions
                    ...
                    for line in handle:
                        ...
                        try:
                            ...
                            if isinstance(entry, dict):
                                m_sid = re.search(r'([a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12})', str(log_file))
                                if m_sid:
                                    entry['sessionId'] = m_sid.group(1)
...
def summarize_long_transcript(messages, max_chars=10000):
    full_text = '\n'.join([f"{m.get('type', 'UNK').upper()}: {str(m.get('message') or m.get('content') or m.get('thinking') or '')[:1000]}" for m in messages])
...
def process_sessions(raw_sessions, logs, drop_noise=False):
    ...
    for session in raw_sessions:
        ...
        user_msgs = [m for m in messages if m.get('type') in ('user', 'human', 'USER', 'USER_INPUT')]
        if len(user_msgs) < 2:
            if drop_noise:
                filtered_count += 1
                continue
        ...
        try:
            ...
            for message in messages:
                ts_raw = message.get('timestamp') or message.get('created_at', '')
        ...
        processed.append({
            ...
            'tokens': sum(len(str(m.get('message') or m.get('content') or m.get('thinking') or '')) for m in messages) // 4,
