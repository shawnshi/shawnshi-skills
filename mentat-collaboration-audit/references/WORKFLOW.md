...
仅综合复盘：证据门通过后读取 [ANALYSIS.md](ANALYSIS.md)，按任务收敛、用户介入、返工、自主性校准、维护价值、委派收益、上下文／规则负担、改进实验八个视角中有证据且相关的视角分析，不凑齐视角。先回答已验收与未收敛任务、必要决策、已证实的纠错／恢复负担及上次修复后续，再引用运行统计。

定点诊断沿用 [SKILL.md 的窄输出](../SKILL.md#输出结构)，只分析该问题与必要事件，不因读取事件合同而加载完整分析或模板。两类任务均保留失败、阻塞、放弃和未完成样本，不只挑成功记录；下述证据、严格校验与覆盖要求同样适用。
...
仅综合复盘：在当前答复中先回答报告模板的六个结果问题，各结论旁列来源声明／已核验出处、任务／介入／返工／cohort 分母与未知，再给有证据且相关视角的发现及支撑运行指标；完整报告模板位于 [report-template.md](report-template.md)。

定点诊断按 SKILL 的窄输出交付，不补六问或八视角章节；仍在结论旁列证据、分母、覆盖与未知，历史 Token 缺计量证据时报告不可计算，不现场采样反填。

两类任务形成行动时读 [IMPROVEMENT.md](IMPROVEMENT.md)，仅选择下一项最小 SYSTEM 或 USER 实验，保留必要人类决策及安全门。只有用户明确要求文件、仪表盘或整改时才产生对应写入；定点诊断不豁免下述保存与验证门。

仅综合复盘使用完整模板：缺证据时明确显示不可用／未知，不是完成审计。可在原有 `--markdown-template`／`--html-template` 中依据已核验的显式证据填写六问、八视角及覆盖；保留建议占位符，不分别手写建议表。

旧聚合 JSON 验证器不认证 `collaboration_analysis` 扩展或保存后的篡改；应从授权输入严格聚合并人工核验指针。`report_pair.py` 不消费该扩展自动填报，只验证建议同源及提交完整性，不认证正文事实；没有新增报告 API。
