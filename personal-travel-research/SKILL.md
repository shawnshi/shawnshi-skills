...
**严禁并行拉起子代理：**
受限于 API 熔断和并发写盘灾难，**禁止**调用 `invoke_subagent`。
对于需要提取知识点的 URL，主代理直接在当前会话中逐个串行执行抓取或搜索。结果提取后直接留存在内存上下文中，不要进行任何物理中转文件落地。

**降级规则：**
- 若某个 URL 获取失败，直接忽略，不阻塞主流程。
- 将收集到的提炼内容在内存中汇总。
...
- JSON 结构：`{"skill_name": "personal-travel-research", "status": "success", "duration_sec": [ESTIMATE], "input_tokens": [ESTIMATE], "output_tokens": [ESTIMATE]}`
