...
2.  **颗粒度控制**:
    ...
    *   仅需基本面时使用 `--info-only`。
    *   这能有效减少 Token 消耗。
3.  **利用机构级摘要统计**: JSON 输出中的 `summary` 字段已包含 `ma5`, `ma10`, `ma20`, `bias_ma5_pct`(乖离率), `max_drawdown_pct`(最大回撤), `dd_from_high_pct`(高点距离), `avg_close`(均价) 等指标，**切勿自行让大模型通过截断的历史数据进行复杂计算**。
