# Layouts · 风格 C 极简医疗风 (Consulting Grade)

基于 22 个严格模块化网格，融合了咨询公司级别的美学组件与双态引擎。

> ⚠️ 这套版式与风格 A(电子杂志) 和 风格 B(瑞士风) **不通用**。这是一套深度适配医疗 IT 战略宣讲与咨询报告的高级风格。

---

## Winning Consulting 模式 (必读)

生成正文页时，不仅可以使用基础的 S01-S22 骨架，还必须融入高级的咨询组件。

**核心变革：DOM 自动降级双态引擎**
1. **无需双写**：大模型生成时，只需按标准输出一份 `<section class="slide">` 内容。
2. **底层魔法**：底层引擎通过 `body.doc-mode` 和 `body.pres-mode` 切换控制。在阅读模式下，所有幻灯片会被自动平铺成带锚点导航的瀑布流长文。
3. **内容密度要求**：正因为支持长文阅读模式，请确保内容的信息密度足够高，不要吝啬专业词汇与详细释义。在段落中适当使用 `<p class="body-sm">` 补充深度细节。

**专属咨询组件 (参见 `components.md`)**：
遇到需要展示架构、路线图、或大量核心指标对比时，**请优先使用**：
- Architecture Layers (`.s-arch`)
- Era Roadmap (`.s-era-grid`)
- Stacked Ledger (`.stacked-ledger`)
...
## 4. 咨询专属框架布局 (Consulting Primitives)

除了上述的通用网格布局外，针对战略汇报、商业咨询场景，引入了符合 MBB（麦肯锡、BCG、Bain）规范的专属分析布局。

### 4.1 强断言表头布局 (Action Title + Breadcrumb)

**结构说明**：
不再使用普通的 `<h2>` 居中，而是使用 `.c-header` 包裹，上方放置追踪导航（`.c-tracker`），下方放置结论性长标题（`.c-action-title`）。

**使用场景**：
几乎所有内页都可以（且应该）使用。

### 4.2 结构化支柱 (MECE Pillars)

**结构说明**：
使用 `.c-pillars` 容器，内部放置 3-4 个 `.c-pillar`，实现并列方案对比或核心举措展开。

**使用场景**：
- 战略三大支柱。
- 实施路径的三阶段总结。
- 替代方案的 MECE（相互独立、完全穷尽）分解。

### 4.3 2x2 定位矩阵 (2x2 Positioning Matrix)

**结构说明**：
使用 `.c-matrix-wrapper` 包含坐标轴 `.c-axis-x/y` 和核心 `.c-matrix` 区域。
内部划分为四个象限 `.c-matrix-quad`。通过给特定的 quadrant 添加 `.sweet-spot` 可高亮"战略重心"。
在矩阵内部放置 `.c-matrix-item` 并通过绝对定位调整位置。

**使用场景**：
- 波士顿矩阵（增长率 vs 市场份额）。
- 实施优先级矩阵（实施难度 vs 业务价值）。
- 竞争格局定位（价格 vs 质量）。
