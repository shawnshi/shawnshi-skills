...
2. **Delegation**: Explicitly invoke a sub-agent (e.g., `worker sub-agent`) or multiple sub-agents in a separate session loop to read the arena packet, conduct the rigorous multi-turn debate, and write a consolidated `Vulnerability_Autopsy_Report.md`.
...
4.  **写入节点**：使用 `write_file` 写入 `00_init.md`，包含议题原子化断言（Assertions）与承重墙识别。调用 `user-input gate` 获取启动确认。
...
1.  **物理合并**：使用 `run_shell_command` 执行 `python C:\Users\shich\.gemini\skills\cognitive-logic-adversary\scripts\session_merger.py "工作区绝对路径" "目标文件绝对路径"`。
