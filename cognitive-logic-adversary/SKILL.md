...
1. **Arena Creation**: Before starting the debate (Phase 2), write the core thesis, premises, and the specific personas to a physical sandbox file: `C:\Users\shich\.gemini\tmp\playgrounds\Arena_Packet_[TIMESTAMP].md`.
2. **Delegation**: 明确调用原生的 `invoke_subagent` 工具，唤醒一个或多个独立子代理，让其读取竞技场包并执行多轮辩论，最终产出 `Vulnerability_Autopsy_Report.md`。
...
2.  **模式路由**：根据用户需求选择作战模式：
    ...
    - **Attack (红队)**: 32 代理饱和攻击。
3.  **写入节点**：使用 `write_file` 将议题原子化断言写入 `C:\Users\shich\.gemini\MEMORY\roundtable\workspace_adversary_{议题关键词}_{date}\00_init.md` (目录会自动创建)。
...
2.  **碎片化落盘**：每一轮质疑与博弈结束后，直接使用 `write_file` 将内容写入工作区内的 `01_round1.md`, `02_round2.md` 等碎片文件。
