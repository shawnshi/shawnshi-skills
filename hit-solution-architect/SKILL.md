...
1. 产出 `Solution_Skeleton.md`，使用原生 `write_to_file` 保存至绝对路径 `C:\Users\shich\.gemini\MEMORY\raw\solutions\`。
2. **并发组装 (Native Delegation)**: 对于 `proposal` / `blueprint` 模式，主代理必须调用 `invoke_subagent` 工具 (指定 `TypeName: "self"` 与各章节独立 Role)，将不同章节的写作任务并发分发给多个子代理。**关键硬锁**：在传给子代理的 `Prompt` 中，主代理必须明确指示子代理将其负责的章节使用 `write_to_file` 工具落盘至隔离防线（例如 `C:\Users\shich\.gemini\MEMORY\scratch\solutions\{方案名称}\chapter_{X}.md`），以备后续集成。
3. **红队刺客逻辑审查 (Red Team Audit)**: 全篇草稿完成后，主代理必须调用 `invoke_subagent` 拉起子代理 (必须指定 `TypeName: "self"` 与 `Role: "cognitive-logic-adversary"`)，对全篇进行矛盾稽查（重点查杀时间线冲突、TCO账目不平）。
...
3. 最终集成与落盘：
   为了合并所有通过校验的章节，主代理必须使用 `write_to_file` 在 `MEMORY\scratch\solutions\` 目录下创建一个 `manifest.json`，结构如 `{"chapters": ["chapter1.md", "chapter2.md"]}`（路径相对于该 json 文件）。然后执行集成命令，合并成最终文档并落盘：
   `$env:PYTHONIOENCODING="utf-8"; python "C:\Users\shich\.gemini\config\skills\hit-solution-architect\scripts\manifest_manager.py" "C:\Users\shich\.gemini\MEMORY\scratch\solutions\manifest.json" "C:\Users\shich\.gemini\MEMORY\raw\solutions\{方案名称}_Final.md"`
...
- **Telemetry 记录**: 使用 `write_to_file` 将执行元数据保存到：
