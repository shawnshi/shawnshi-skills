...
2. **并发组装 (Native Delegation)**: 对于 `proposal` / `blueprint` 模式，主代理必须调用 `invoke_subagent` 工具，将不同章节的写作任务并发分发给多个子代理。**关键硬锁**：在传给子代理的 `Prompt` 中，主代理必须明确指示子代理将其负责的章节使用 `write_to_file` 工具落盘至隔离防线（例如 `C:\Users\shich\.gemini\MEMORY\scratch\solutions\{方案名称}\chapter_{X}.md`），以备后续集成。
3. **红队刺客逻辑审查 (Red Team Audit)**: 全篇草稿完成后，主代理必须调用 `invoke_subagent` 拉起 `cognitive-logic-adversary` 子代理，对全篇进行矛盾稽查（重点查杀时间线冲突、TCO账目不平）。
