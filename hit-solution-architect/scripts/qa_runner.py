...
def build_report(
    ...
    *,
    ...
    errors = _tag(logic_report["errors"], "logic") + _tag(style_report["errors"], "style")
    ...
    review = _tag(logic_report["review"], "logic") + _tag(style_report["review"], "style")
    ...
    return {
        ...
        "gate": {
            ...
            "release_ready": not errors and review_complete if stage == "release" else None,
        },
        "errors": errors,
        "warnings": warnings,
        "review": review,
...
def _argument_failure(target: str, message: str) -> int:
    print(render_report(runtime_failure_report("qa_runner", target, "E_ARGUMENT", message)))
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if unknown_modules:
        return _argument_failure(target, f"未知 --require 模块：{', '.join(unknown_modules)}。")
    if args.bold_hint < 0:
        return _argument_failure(target, "--bold-hint 必须大于或等于 0。")
    if not 0 <= args.max_warnings <= MAX_WARNING_LIMIT:
        return _argument_failure(target, f"--max-warnings 必须在 0 到 {MAX_WARNING_LIMIT} 之间。")
    ...
    if args.output:
        try:
            ...
            report = runtime_failure_report("qa_runner", target, "E_FILE_WRITE", str(exc))
