...
> 📌 人工评估（上表 5 维）适用于定性审阅；自动评估（`metrics/quality_scoring.py` 7 维）适用于可量化的文本特征检测。两者互补，不可替代。

| 人工评估维度 (5D) | 自动评估维度 (`quality_scoring.py` 7D) | 关系 |
|---|---|---|
| **逻辑严密性** (30%) | — | 仅人工可判断 |
| **证据质量** (20%) | — | 仅人工可判断 |
| **语言质量** (20%) | 动词密度 (20%) + 黑话检测 (20%) + 冗余副词 (15%) + 可读性 (15%) + 被动语态 (10%) | 自动评估覆盖语言质量的多个子维度 |
| **诚实性** (15%) | 分析师手记 (15%) | 自动检测存在性与基本质量 |
| **创新性** (15%) | — | 仅人工可判断 |
| — | 粗体使用 (5%) | 自动检测格式规范 |

### 自动化质量评分工具

已实现的自动化检测工具（见 `metrics/quality_scoring.py`），运行方式：
```bash
