...
# Cognitive Personal Roundtable (V8.1 Native)
...
2. **Delegation**: Explicitly invoke a sub-agent using the `invoke_subagent` tool to read the packet, execute the actual conversational turns, and append the dialog fragments to the designated physical output workspace.
...
2.  **工作区初始化 (Mandatory)**：
    - 直接使用 `write_file` 工具将第一个碎片文件 `00_init.md` 写入 `C:\Users\shich\.gemini\MEMORY\raw\roundtable\workspace_{议题关键词}_{date}\00_init.md` (目录会自动创建)。
...
3.  **碎片化落盘 (Anti-Dehydration)**：
    - 每轮结束后，直接使用 `write_file` 工具将本轮内容写入 `01_round1.md`, `02_round2.md`... 等文件。
...
- **未验证的假工具**：严禁伪造 `run_shell_command` 或 `user-input gate`。交互必须走 `ask_question`，终端命令必须走 `run_command`。
