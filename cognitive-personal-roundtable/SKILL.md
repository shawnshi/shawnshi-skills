...
2. **合并重组**: 子代理进行全局总结后，主代理必须调用 `scripts/merger.py` (`python C:\Users\shich\.gemini\config\skills\cognitive-personal-roundtable\scripts\merger.py <workspace_path> <output_file_path>`) 将沙盒内散落的轮次碎片文件 (00_*.md, 01_*.md 等) 进行物理合并，生成完整的会议归档总结报告。
