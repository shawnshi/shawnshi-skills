...
# Cognitive Personal Roundtable (V5.0: Mentat Synthesis Edition)
...
1.  **动态选人 (Tension Grid)**：根据议题，从 `C:\Users\shich\.gemini\skills\cognitive-personal-roundtable\references\personas.md` 中选择或自定义 3-5 位人物，构建**张力网络**，至少包含一位"意外视角"。
2.  **工作区初始化 (Mandatory)**：
...
2.  **主持人综述 (Mentat Recap)**：
    - **核心争议点**：精准定位逻辑裂缝。
    - **ASCII 思考框架图**：参考 `C:\Users\shich\.gemini\skills\cognitive-personal-roundtable\templates\ascii_topology.md` 选择最贴合的形式。
    - **下潜问题**：引导后续讨论。
...
2.  **物理合并 (The Merge)**：
    - 使用 `run_shell_command` 执行 `python C:\Users\shich\.gemini\skills\cognitive-personal-roundtable\scripts\merger.py "工作区绝对路径" "目标文件绝对路径"`。
...
- JSON 结构：`{"skill_name": cognitive-personal-roundtable", "status": "success", "duration_sec": [ESTIMATE], "input_tokens": [ESTIMATE], "output_tokens": [ESTIMATE]}`
