...
1.  **Project Intake Gate (项目启动收口)**: 需要确认关键信息时，使用 `notify_user` (BlockedOnUser: true) 结合 `Message` 字段一次性向用户聚合并明确关键叙事主轴：
...
4.  **Final Review (交付关卡)**：实证加固后，撰写一页纸核心摘要（背景、洞察、卫生经济学影响、下一步行动），连同末尾强制的 `> ⚠️ Clinical & Regulatory Constraint:` 风险披露，**使用 `notify_user` (BlockedOnUser: true) 提交最终成果给用户验收**。
