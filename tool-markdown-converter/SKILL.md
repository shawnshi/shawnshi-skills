...
- `uvx --with "markitdown[all]" markitdown`
- 输入文件与输出 Markdown 文件

## Failure Modes
- 不要直接对二进制文件做原始读取。
- 若 `uv` 或插件缺失，必须先修复依赖。
- ZIP 转换后若内容过长，要提醒上下文限制。
...
- JSON 结构：`{"skill_name": "tool-markdown-converter", "status": "success", "duration_sec": [ESTIMATE], "input_tokens": [ESTIMATE], "output_tokens": [ESTIMATE]}`
