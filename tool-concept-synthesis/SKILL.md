...
Strategy:
1. 算力下沉：强制调用 vector-lake-mcp 获取底层的拓扑关联，严禁大模型自己盲搜文本。
...
# Tool Concept Synthesis (宏观缝合与体系全景图 V8.1 Native)
...
3. **调用 Vector Lake**：通过原生 `call_mcp_tool` 调度 `vector-lake` 服务器的 `search_vector_lake` 或 `query_logic_lake` 工具，提取底层的实体拓扑、权重边以及带有来源出处的 Claims。
