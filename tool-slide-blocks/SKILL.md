---
name: tool-slide-blocks
description: PPT 智能组装助手。Primary owner for slide-library retrieval and assembly: find existing pages, reuse historical PPT assets, swap templates, and assemble decks from source slides. Prefer tool-slide-architect when the task is to design a new strategy-deck narrative or chapter architecture from scratch.
...
- **Mode D(跨技能联合编排 )**：当遇到宏大的战略汇报任务时，`tool-slide-blocks` 应退格为底层的“渲染引擎”，与 `tool-slide-architect` (大脑) 组成联合流水线。
...
当遇到宏大的战略汇报任务时，`tool-slide-blocks` 应退格为底层的“渲染引擎”，与 `tool-slide-architect` (大脑) 组成联合流水线：

1. **大脑定调**：由 `tool-slide-architect` 主导 OODA 推演，输出带有 `[Visual_Intent]` 标记的 Markdown 骨架。
