...
- Use `academic-deep-research` (full mode) for comprehensive literature exploration
- Or use `academic-paper-writer` (plan mode) to start planning the paper directly
```
...
- The Research Plan Summary can be handed directly to `academic-paper-writer` (plan mode)
- If the user wants deeper literature exploration, suggest switching to `academic-deep-research` (full mode)
- `academic-paper-writer`'s `intake_agent` will automatically detect an existing Research Plan Summary and skip redundant steps
