...
# Tool Document Summarizer (Medical Intelligence Edition)
...
**CRITICAL RULE**: To protect the main agent's context window from attention degradation and data bloat when processing massive documents (PDFs, PPTXs, DOCXs), the extraction and summarization MUST NOT be executed directly in the main memory.
1. **Packet Creation**: Before starting the summarization task, write the target file paths and specific extraction queries to a physical sandbox file: `C:\Users\shich\.gemini\tmp\playgrounds\Summary_Packet_[TIMESTAMP].md`.
2. **Delegation**: Explicitly invoke a sub-agent (e.g., `worker sub-agent` or a dedicated parser) to consume the packet, perform the heavy document ingestion, and write the structured summary back to a designated result file.
3. **Suspension**: The main agent must suspend execution, wait for the sub-agent to complete the extraction, and then read ONLY the final result file to deliver the strategic insights.
...
- 若处理目录级任务，必须明确指出 `output/` 下的核心产物，并给出对用户有用的简要战略结论，而不是只给文件路径。
